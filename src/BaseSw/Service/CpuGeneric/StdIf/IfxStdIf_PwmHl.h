...
/** \brief Return the dead time ticks in s
 *
 * It returns the last dead time tick values set by IfxStdIf_PwmHl_SetDeadtime() or during initialisation
 * \param stdIf Pointer to the interface driver object
 * \return Returns the deadtime in second
 */

typedef float32 (*IfxStdIf_PwmHl_GetDeadtimeTick)(IfxStdIf_InterfaceDriver stdIf);
...
struct IfxStdIf_PwmHl_
{
	...
	IfxStdIf_PwmHl_GetDeadtimeTick getDeadtimeTick;                 /**< \breif IfxStdIf_PwmHl_GetDeadtimetick */
 ...
 * \param stdIf Standard interface pointer
 */
IFX_INLINE float32 IfxStdIf_PwmHl_getDeadtimetick(IfxStdIf_PwmHl *stdIf)
{
    return stdIf->getDeadtimeTick(stdIf->driver);
}
