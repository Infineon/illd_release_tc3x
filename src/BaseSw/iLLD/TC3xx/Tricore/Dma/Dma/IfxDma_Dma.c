...
IFX_STATIC void IfxDma_Dma_configureTransactionSet(Ifx_DMA_CH *channel, const IfxDma_Dma_ChannelConfig *config)
{
    ...
    /* Configure the channel configuration register (CHCFGR) */
    {
    ...
    /* Configure the address and interrupt control register (ADICR) */
    {
    ...
    /* Configure the transaction status register (TSR) */
    {
        Ifx_DMA_TSR tsr;
...
{
	/* Get the DMA module pointer from the configuration */
    Ifx_DMA *dma = config->module->dma;

    /* Assign the DMA module and channel ID to the channel handle */
    channel->dma       = dma;
    ...
    /* Configure the transaction status register (TSR) */
    {
        ...
        /* Enable or disable hardware requests based on the configuration */
...
{
	/* Define a default DMA channel initialization configuration structure */
