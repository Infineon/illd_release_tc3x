...
boolean IfxGtm_Tim_In_init(IfxGtm_Tim_In *driver, const IfxGtm_Tim_In_Config *config)
{
    ...
    if (config->timeout.timeout > 0)
    {
        ...
        channel->TDUV.U       |= (0xFFFFFF & timeout);
...
{
	/* Check if data was lost and clear the event flag */
    ...
    /* Check for new data availability and process it */
    ...
    if (driver->newData)
    {
        ...
        /* Check for counter overflow and clear the event flag */
        ...
        /* Clear the new value event flag */
