...
/** \brief Bit compression input setup for TIM_IN channel
 */
typedef struct
{
    IfxGtm_Tim_In *inputChannel;       /**< \brief Pointer to input channel handle */
    boolean        enableRisingEdge;   /**< \brief TRUE to enable rising edge from the input channel */
    boolean        enableFallingEdge;  /**< \brief TRUE to enable falling edge from the input channel */
} IfxGtm_Tim_In_BitCompressionInput;
...
/** \brief Configures selected TIM input channels for bit-compression mode
 *
 * \param[inout] driver             TIM Input object
 * \param[in]    inputSetupArray    Array of input channel setup entries
 * \param[in]    numOfInputChannels Number of entries in inputSetupArray. Range: 1 to \ref IFXGTM_NUM_TIM_CHANNELS
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_setBitCompressionInputs(IfxGtm_Tim_In *driver, IfxGtm_Tim_In_BitCompressionInput *inputSetupArray, uint8 numOfInputChannels);

/** \brief Gets the bit-compressed data from the TIM input channel
 *
 * \param[in] driver TIM Input object
 *
 * \retval uint8 Bit-compressed data
 */
IFX_INLINE uint8 IfxGtm_Tim_In_getBitCompressionData(IfxGtm_Tim_In *driver);

/** \brief Disable TIM Channel
 *
 * \param[inout] driver Pointer to the TIM Input handle
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_disableChannel(IfxGtm_Tim_In *driver);

/** \brief Enable TIM Channel
 *
 * \param[inout] driver Pointer to the TIM Input handle
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_enableChannel(IfxGtm_Tim_In *driver);

/** \brief Updates the GPR1 register for dynamic period changes in gated periodic sampling mode.
 *
 * \param[inout] driver Pointer to the TIM Input handle
 * \param[in]    value  New GPR1 value for the shadow counter update. Range: 0 to 0xFFFFFF
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_updateGpr1(IfxGtm_Tim_In *driver, uint32 value);
...
IFX_INLINE void IfxGtm_Tim_In_setBitCompressionInputs(IfxGtm_Tim_In *driver, IfxGtm_Tim_In_BitCompressionInput *inputSetupArray, uint8 numOfInputChannels)
{
    ...
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, (numOfInputChannels > 0u));

    for (index = 0u; index < numOfInputChannels; index++)
    {
        /* Input channel index must be within TIM channel range */
        IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, ((uint32)inputSetupArray[index].inputChannel->channelIndex < IFXGTM_NUM_TIM_CHANNELS));

        /* Build per-channel CNTS mask: bit ch (rising) and bit m+ch (falling) */
        mask |= (uint32)(((1u << IFXGTM_NUM_TIM_CHANNELS) | 1u) << inputSetupArray[index].inputChannel->channelIndex);

        /* Build CNTS bit selection: bits [0..m-1] rising, bits [m..2m-1] falling */
        if (inputSetupArray[index].enableRisingEdge == TRUE)
        {
            value |= (uint32)(1u << inputSetupArray[index].inputChannel->channelIndex);
        }

        if (inputSetupArray[index].enableFallingEdge == TRUE)
        {
            value |= (uint32)((1u << IFXGTM_NUM_TIM_CHANNELS) << inputSetupArray[index].inputChannel->channelIndex);
        }
    }

    driver->channel->CTRL.B.TIM_MODE = (uint8)IfxGtm_Tim_Mode_bitCompression;
    Ifx__ldmst(&driver->channel->CNTS, mask, value);
}


IFX_INLINE uint8 IfxGtm_Tim_In_getBitCompressionData(IfxGtm_Tim_In *driver)
{
    return (uint8)driver->channel->GPR1.B.GPR1;
}


IFX_INLINE void IfxGtm_Tim_In_disableChannel(IfxGtm_Tim_In *driver)
{
	driver->channel->CTRL.B.TIM_EN = 0U;
}


IFX_INLINE void IfxGtm_Tim_In_enableChannel(IfxGtm_Tim_In *driver)
{
	driver->channel->CTRL.B.TIM_EN = 1U;
}


IFX_INLINE void IfxGtm_Tim_In_updateGpr1(IfxGtm_Tim_In *driver, uint32 value)
{
    driver->channel->GPR1.B.GPR1 = value;
}
