 ...
 *
 * \param[inout] driver Pointer to GTM ATOM PWM driver.
 ...
 *
 * \param[in] driver Pointer to GTM ATOM PWM driver.
 *
 * \retval None
 */
...
boolean IfxGtm_Atom_Dtm_PwmHl_init(IfxGtm_AtomDtm_PwmHl *driver, const IfxGtm_Atom_Dtm_PwmHl_Config *config)
{
    ...
    for (channelIndex = 0; channelIndex < config->base.channelCount; channelIndex++)
    {
        ...
        driver->dtm[channelIndex] = IfxGtm_Dtm_getDtmPointer(timer->gtm, IfxGtm_Dtm_getCdtmIndexForAtom(config->atom), IfxGtm_Dtm_getDtmIndexForAtom(config->atom, config->ccx[channelIndex]->channel));
        ...
        if (driver->dtm[channelIndex] != NULL_PTR)
        {
            ...
            IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, IfxGtm_Dtm_getDtmChannelForAtom(config->ccx[channelIndex]->channel) == IfxGtm_Dtm_getDtmChannelForAtom(config->coutx[channelIndex]->channel));

            IfxGtm_Dtm_setClockSource(driver->dtm[channelIndex], config->deadTimeClock);
            driver->dtmChannel[channelIndex] = IfxGtm_Dtm_getDtmChannelForAtom(config->ccx[channelIndex]->channel);
    ...
    {
        driver->dtmClockFreq = IfxGtm_Dtm_getClockFrequency(timer->gtm, IfxGtm_Dtm_getCdtmIndexForAtom(config->atom), IfxGtm_Dtm_getDtmIndexForAtom(config->atom, config->ccx[0]->channel));
...
void IfxGtm_Atom_Dtm_PwmHl_setOnTime(IfxGtm_AtomDtm_PwmHl *driver, Ifx_TimerValue *tOn)
{
