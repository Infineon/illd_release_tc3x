 ...
 *
 *
 * \defgroup IfxLld_Gtm_Pwm_Usage How to use the GTM PWM Driver
 * \ingroup IfxLld_Gtm_Pwm
 *
 * This driver implements the PWM functionalities.
 *
 ...
 *
 * \code
 * #define IFXGTM_CH0CCU0_INTR_PRIO 3
 * #define IFXGTM_PWM_NUM_CHANNELS 4
 * #define IFXGTM_PWM_NUM_CHANNELS 4
 * #define IFXGTM_PWM_INITIAL_FREQUENCY (500000.0f)  // 500 KHz
 *
 * Ifx_GTM                     *gtmSFR = &MODULE_GTM;
 *
 * IfxGtm_Dtm_ClockSource      dtmClockSource = IfxGtm_Dtm_ClockSource_cmuClock0;
 * IfxGtm_Pwm_Channel          channels[IFXGTM_PWM_NUM_CHANNELS];
 * IfxGtm_Pwm_SubModule_Ch     channelIndex;
 * float32 reqDuty[IFXGTM_PWM_NUM_CHANNELS]   = {50.0f, 25.0f, 37.5f, 50.0f};
 *
 * \endcode
 *
 ...
 *
 * // enable GTM clock
 * float32 frequency = IfxGtm_Cmu_getModuleFrequency(gtmSFR);
 * IfxGtm_Cmu_setGclkFrequency(gtmSFR, frequency);
 * IfxGtm_Cmu_enableClocks(gtmSFR, IFXGTM_CMU_CLKEN_ALL); // Enable all clocks
 *
 * //PWM Configuration
 ...
 * IfxGtm_Pwm_OutputConfig     output[IFXGTM_PWM_NUM_CHANNELS];
 * // Update the Output config as per requirement.
 * Eg:
 * output[0].pin                    = (IfxGtm_Pwm_ToutMap*)&IfxGtm_TOM1_5_TOUT11_P00_2_OUT;
 ...
 *
 * // DTM Configuration
 * IfxGtm_Pwm_DtmConfig        dtmConfig[IFXGTM_PWM_NUM_CHANNELS];
 * // Update the DTM config as per requirement.
 * Eg:
 * dtmConfig[0].deadTime.rising = 0.0f;
 * dtmConfig[0].deadTime.rising = 0.0f;
 *
 * // Base channel interrupt configuration
 * IfxGtm_Pwm_InterruptConfig  interruptConfig;
 * // Update the channel interrupt configuration (like mode,isrProvider,priority,etc) as per requirement.
 *
 * // Update channel config as per requirement.
 * //Eg:
 * channelConfig[0].timerCh    = IfxGtm_Pwm_SubModule_Ch_5;
 * channelConfig[0].phase      = 0.0f;
 * channelConfig[0].duty       = 75.0f;
 * channelConfig[0].dtm        = &dtmConfig[0];
 * channelConfig[0].output     = &output[0];
 * channelConfig[0].mscOut     = NULL_PTR;
 * channelConfig[0].interrupt  = &interruptConfig;
 *
 * // Update other Pwm config (like cluster, dtmClockSource,numChannels, etc) as per requirement .
 ...
 * config.numChannels          = IFXGTM_PWM_NUM_CHANNELS;
 * config.channels             = &channelConfig[0];
 * config.frequency            = IFXGTM_PWM_INITIAL_FREQUENCY;
 * config.dtmClockSource       = dtmClockSource;
 * config.syncUpdateEnabled    = TRUE;
 *
 * // Initilizatise the PWM
 * IfxGtm_Pwm_init(&pwm, &channels[0], &config);
 *
 * // To update frequency
 * IfxGtm_Pwm_updateFrequency(&pwm, IFXGTM_PWM_REQUEST_FREQUENCY);
 *
 * // Wait one cycle for registers to be updated
 * ch0ccu0InterruptCount = 0u;
 * while (ch0ccu0InterruptCount < 1)
 * {}
 *
 *  // To update Duty
 ...
 * \endcode
 *
 * \defgroup IfxLld_Gtm_Pwm PWM Interface Driver
 * \ingroup IfxLld_Gtm
 */
...
/**
 * \brief Configures the polarity of a PWM channel's signal level during its duty cycle.
 *
 * \param[inout] clusterSFR Pointer to the Cluster object.
 * \param[in]  	 subModule  Submodule selection, either ATOM or TOM module of the GTM cluster. Range: \ref IfxGtm_Pwm_SubModule
 * \param[in] 	 channel    PWM channel index to configure. Range: \ref IfxGtm_Pwm_SubModule_Ch
 * \param[in] 	 polarity   Active state of the PWM signal. Range: \ref Ifx_ActiveState
 *
 * \retval None
 */
 ...
 *
 * \param[inout] pwm          Pointer to PWM handle.
 * \param[in] 	 configIndex  Index of channel in the array of configured PWM channels. Range: \ref IfxGtm_Pwm_SyncChannelIndex
 * \param[in] 	 requestPhase Requested phase value in radians. Range: 0.0 to 2 * pi
 *
 * \retval None
 *
 * \note Use this API only for sync channels and not for base channel. API doesn't check this condition.
 */
 ...
 * \param[inout] pwm          Pointer to PWM handle.
 * \param[in]    configIndex  Index of channel in the array of configured PWM channels. Range: \ref IfxGtm_Pwm_SyncChannelIndex
 * \param[in]    requestDuty  Requested duty in % . Range: 0.0 to 100.0
 *
 * \retval None
 */
 ...
 * \param[in]    gtmSFR Pointer to GTM module.
 *
 * \retval None
 */
 ...
 *
 * \param[inout] channelConfig Pointer to Channel Configuration structure.
 *
 * \retval None
 */
 ...
 * 
 * \param[inout] pwm      Pointer to PWM handle.
 * \param[in]    channels Pointer to array to store channel data.
 ...
 * 
 * \param[inout] pwm1 Pointer to PWM handle 1.
 * \param[inout] pwm2 Pointer to PWM handle 2.
 *
 * \retval None
 */
 ...
 * 
 * \param[inout] pwm              Pointer to PWM handle.
 * \param[in]    requestFrequency Requested frequency in Hz. Range: Must be a positive value (frequency > 0).
 *
 * \retval None
 */
 ...
 * 
 * \param[inout] pwm 	      Pointer to PWM handle.
 * \param[in]    configIndex  Enum to choose channel type (base/sync) and index. Range: \ref IfxGtm_Pwm_SyncChannelIndex
 * \param[in]    requestPhase New phase value (applicable for edge aligned sync channel).
 * \param[in]    requestDuty  New duty value. Range: 0.0 to 100.0
 *
 * \retval None
 */
 ...
 * 
 * \param[inout] pwm          Pointer to PWM handle.
 * \param[in]    requestPhase Pointer to New phase values.
 * \note First index element is ignored
 *
 * \retval None
 */
 ...
 * 
 * \param[inout] pwm Pointer to PWM handle.
 * \param[in]    requestDuty Pointer to New duty value.
 *
 * \retval None
 */
 ...
 * 
 * \param[inout] pwm          Pointer to PWM handle.
 * \param[in]    requestPhase Pointer to New phase value(applicable for edge aligned sync channel).
 * \param[in]    requestDuty  Pointer to New duty value.
 *
 * \retval None
 */
 ...
 * 
 * \param[inout] pwm             Pointer to PWM handle.
 * \param[in]    requestDeadTime Pointer to New dead time value.
 *
 * \retval None
 */
 ...
 * \param[inout] pwm          Pointer to PWM handle.
 * \param[in]    requestPhase Pointer to New phase value(applicable for edge aligned sync channel).
 * \param[in]    requestDuty  Pointer toNew duty value.
 *
 * \retval None
 */
 ...
 * \param[in]    data    Pointer to Data to pass to callback functions.
 *
 * \retval None
 */
 ...
 * 
 * \param[in] pwm Pointer to PWM handle.
 * \param[in] channel Channel index. Range: \ref IfxGtm_Pwm_SubModule_Ch
 *
 * \retval IfxGtm_Pwm_ChannelState Channel state. Range: \ref IfxGtm_Pwm_ChannelState
 */
 ...
 * \retval None
 */
IFX_EXTERN void IfxGtm_Pwm_stopChannelOutputs(IfxGtm_Pwm *pwm);
 ...
 * \retval None
 */
IFX_EXTERN void IfxGtm_Pwm_startChannelOutputs(IfxGtm_Pwm *pwm);

/**
 * \brief Resets the fast-shutoff status for all PWM channels in the PWM group.
 * Reset takes effect only for channels where the fast-shutoff condition has already cleared.
 *
 * \param[inout] pwm Pointer to PWM handle.
 *
 * \retval None
 */
IFX_EXTERN void IfxGtm_Pwm_resetFastShutoff(IfxGtm_Pwm *pwm);

/**
 * \brief Enables automatic reset of fast-shutoff status for all PWM channels in the PWM group.
 * After calling this API, PWM channels resume operation automatically when the fast-shutoff condition clears.
 *
 * \param[inout] pwm Pointer to PWM handle.
 *
 * \retval None
 */
IFX_EXTERN void IfxGtm_Pwm_enableSelfResetOfFastShutoff(IfxGtm_Pwm *pwm);
