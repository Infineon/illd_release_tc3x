 ...
 *
 * \param[in] clusterSFR Pointer to Cluster SFR.
 * \param[in] subModule  GTM Sub Module. Range: \ref IfxGtm_Pwm_SubModule
 * \param[in] channel    PWM channel. Range: \ref IfxGtm_Pwm_SubModule_Ch
 *
 * \retval Ifx_GTM_CDTM_DTM* Returns pointer to DTM if available for given configuration.
 */
...
/**
 * \brief Enable Synchronous update of compare registers from shadow registers for the specified channels.
 *
 * \param[inout] glbCtrl  Pointer to the global control register.
 * \param[in]    upenMask UPEN mask that specifies which channels to enable for synchronous update. Range: 0 to 0xFFFF
 *
 * \retval None
 */
...
/**
 * \brief Disables the synchronous update of compare registers from shadow registers for the specified PWM channels.
 *
 * \param[inout] glbCtrl  Pointer to the global control register.
 * \param[in]    upenMask UPEN Mask of channel specifying which channels to disable the synchronous update for. Range: 0 to 0xFFFF
 *
 * \retval None
 */
...
/**
 * \brief Enables synchronous updates of all configured PWM channels in AGC/TGC0/TGC1.
 *
...
/**
 * \brief Disables synchronous update of all configured channels in AGC/TGC0/TGC1.
 *
 * \param[inout] pwm Pointer to the PWM handle.
 *
 * \retval None
...
/**
 * \brief Enable the ATOM or TOM channel to start the counter when a trigger is received.
 *
 * \param[inout] clusterSFR Pointer to the Cluster object.
 * \param[in]    subModule  Submodule selection (ATOM or TOM). Range: \ref IfxGtm_Pwm_SubModule
 * \param[in]    channel    Channel index to enable. Range: \ref IfxGtm_Pwm_SubModule_Ch
 *
 * \retval None
 */
...
/**
 * \brief Enables the ATOM or TOM channel PWM output
 * 
 * \param[inout] clusterSFR Pointer to the Cluster object.
 * \param[in]    subModule  Submodule selection (ATOM or TOM). Range: \ref IfxGtm_Pwm_SubModule
 * \param[in]    channel    Channel index to enable. Range: \ref IfxGtm_Pwm_SubModule_Ch
 ...
 * \retval None
 *
 * \note Channel starts generating PWM after receiving trigger.
 */
...
/**
 * \brief Sets the ATOM or TOM channel clock source.
 *
 * \param[inout] clusterSFR  Pointer to the Cluster object.
 * \param[in]    subModule   Submodule selection (ATOM or TOM). Range: \ref IfxGtm_Pwm_SubModule
 * \param[in]    channel     Channel index. Range: \ref IfxGtm_Pwm_SubModule_Ch
 * \param[in]    clockSource The clock source to be configured for the specified channel. Range: \ref IfxGtm_Pwm_ClockSource
 *
 * \retval None
 */
...
/**
 * \brief Sets the counter to reset either on CM0 match or when trigger is received.
 *
 * \param[inout] clusterSFR Pointer to the Cluster object.
 * \param[in]    subModule  Submodule selection (ATOM or TOM). Range: \ref IfxGtm_Pwm_SubModule
 * \param[in]    channel    Channel index. Range: \ref IfxGtm_Pwm_SubModule_Ch
 * \param[in]    event      Specifies the event that triggers the counter reset. Range: \ref IfxGtm_Pwm_ResetEvent
 *
 * \retval None
 */
...
/**
 * \brief Sets the counter mode according to PWM signal alignment.
 *
 * \param[inout] clusterSFR Pointer to the Cluster object.
 * \param[in]    subModule  Submodule selection (ATOM or TOM). Range: \ref IfxGtm_Pwm_SubModule
 * \param[in]    channel    Channel index. Range: \ref IfxGtm_Pwm_SubModule_Ch
 * \param[in]    alignment  PWM signal alignment to set. Range: \ref IfxGtm_Pwm_Alignment
 *
 * \retval None
 */
 ...
 * \param[in]    subModule  Submodule selection (ATOM or TOM). Range: \ref IfxGtm_Pwm_SubModule
 * \param[in]    channel    Channel index. Range: \ref IfxGtm_Pwm_SubModule_Ch
 * \param[in]    enabled    Boolean flag to control trigger generation.
 *                          - TRUE: Generate trigger
...
/**
 * \brief Initializes a PWM pin for base or sync channel based on the submodule configuration.
 *
 * \param[inout] pwm Pointer to the PWM handle.
 * \param[in]    pin Pointer to pin configuration structure.
 *
 * \retval None
 */
 ...
 * \param[in]    subModule    Submodule selection (ATOM or TOM). Range: \ref IfxGtm_Pwm_SubModule
 * \param[in]    channel      Channel index. Range: \ref IfxGtm_Pwm_SubModule_Ch
 * \param[in]    compareValue Generate trigger when counter reaches this value. Range: 0 to 0xFFFFFF
 *
 * \retval None
 */
...
/**
 * \brief Updates the base channel frequency with requested value at the end of period (synchronously).
 *
 * \param[inout] pwm              Pointer to the PWM handle.
 * \param[in]    requestFrequency Requested frequency in Hz. The value must be greater than 0
 *                                and less than or equal to the maximum frequency supported by the system. Range: Must be a positive value (frequency > 0).
 *
 * \retval None
 */
...
/**
 * \brief Updates the base channel frequency with requested value immediately (asynchronously).
 * 
 * \param[inout] pwm              Pointer to the PWM handle.
 * \param[in]    requestFrequency Requested frequency in Hz. The value must be greater than 0
 *                                and less than or equal to the maximum frequency supported by the system. Range: Must be a positive value (frequency > 0).
 ...
 * \param[in] gtmSFR      Pointer to GTM SFR.
 * \param[in] clockSource Clock configuration. \ref IfxGtm_Pwm_ClockSource
 *
 ...
 * \param[in] pwm         Pointer to the PWM handle.
 * \param[in] gtmSFR      Pointer to GTM SFR.
 * \param[in] clockSource Clock configuration. Range: \ref IfxGtm_Dtm_ClockSource
 *
 * \retval float32 Clock frequency in Hz.
 */
 ...
 * \param[in]    configIndex Index of channel to be initialized. Range: \ref IfxGtm_Pwm_SyncChannelIndex
 * 
 * \retval None
 */
 ...
 * 
 * \param[inout] pwm       Pointer to the PWM handle.
 * \param[in]    interrupt Pointer to Interrupt configuration.
...
void IfxGtm_Pwm_resetFastShutoff(IfxGtm_Pwm *pwm)
{
    uint8 idx;

    for (idx = 0u; idx < pwm->numChannels; idx++)
    {
        IfxGtm_Pwm_SubModule_Ch channelIndex = pwm->channels[idx].timerCh;
        Ifx_GTM_CDTM_DTM       *dtmSFR       = IfxGtm_Pwm_getDtmPtr(&pwm->clusterSFR, pwm->subModule, channelIndex);

        /* Write SHUT_OFF_RST bit in DTM CTRL to reset SHUTOFF_SYNC */
        IfxGtm_Dtm_resetShutoffSignal(dtmSFR);
    }
}


void IfxGtm_Pwm_enableSelfResetOfFastShutoff(IfxGtm_Pwm *pwm)
{
    uint8 idx;
    ...
    for (idx = 0u; idx < pwm->numChannels; idx++)
    {
        ...
        /* Sets shutoff update mode: signal PSU_SHUT_OFF=1 sets internal signal SHUTOFF_SYNC;
         * signal PSU_SHUT_OFF=0 clears internal signal SHUTOFF_SYNC */
        IfxGtm_Dtm_setShutoffUpdateMode(dtmSFR, IFXGTM_DTM_UPD_MODE_SELF_RESET);
    }
}
...
void IfxGtm_Pwm_stopChannelOutputs(IfxGtm_Pwm *pwm)
{
    ...
    for (idx = 0; idx < pwm->numChannels; idx++)
    {
        ...
        mask |= IFXGTM_PWM_OUTPUT_STAT_DISABLE << (index << 1u);
    }
    ...
    {
    	pwm->clusterSFR.TOM->TGC0_OUTEN_STAT.U = mask & 0xFFFF;
        pwm->clusterSFR.TOM->TGC1_OUTEN_STAT.U = (mask >> 16u) & 0xFFFF;
    }
}
...
void IfxGtm_Pwm_startChannelOutputs(IfxGtm_Pwm *pwm)
{
    ...
    for (idx = 0; idx < pwm->numChannels; idx++)
    {
        ...
        mask |= IFXGTM_PWM_OUTPUT_STAT_ENABLE << (index << 1u);
    }
