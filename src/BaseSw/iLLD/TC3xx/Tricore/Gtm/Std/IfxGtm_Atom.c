...
void IfxGtm_Atom_Agc_enableChannel(Ifx_GTM_ATOM_AGC *agc, IfxGtm_Atom_Ch channel, boolean enabled, boolean immediate)
{
    ...
    /* Build the enable channel control value for the specified channel */
    ...
    /* Update the enable/disable control register */
    Ifx__ldmst(&(agc->ENDIS_CTRL.U), mask, value);

    if (immediate)
    {
    	/* Update the enable/disable status register */
...
void IfxGtm_Atom_Agc_enableChannelOutput(Ifx_GTM_ATOM_AGC *agc, IfxGtm_Atom_Ch channel, boolean enabled, boolean immediate)
{
    ...
    /* Build the enable channel output control value for the specified channel */
    ...
    /* Update the output enable control register */
    Ifx__ldmst(&(agc->OUTEN_CTRL.U), mask, value);

    if (immediate)
    {
    	/* Update the output enable status register */
        Ifx__ldmst(&(agc->OUTEN_STAT.U), mask, value);
    }
}


void IfxGtm_Atom_Agc_enableChannelTrigger(Ifx_GTM_ATOM_AGC *agc, IfxGtm_Atom_Ch channel, boolean enabled)
{
	/* Build the enable channel trigger control value for the specified channel */
	uint32 value = IfxGtm_Atom_Agc_buildFeatureForChannel(channel, enabled, IFX_GTM_ATOM_AGC_INT_TRIG_INT_TRIG0_OFF);
	uint32 shift = IFXGTM_ATOM_AGC_CHANNEL_SHIFT(channel, IFX_GTM_ATOM_AGC_INT_TRIG_INT_TRIG0_OFF);
	uint32 mask  = (uint32)IFXGTM_ATOM_AGC_CHANNEL_MASK << shift;

	/* Update the internal trigger control register */
...
{
	/* Build the enable channel update control value for the specified channel */
    ...
    /* Update the global control register */
...
void IfxGtm_Atom_Agc_enableChannels(Ifx_GTM_ATOM_AGC *agc, uint16 enableMask, uint16 disableMask, boolean immediate)
{
    ...
    /* Build the enable channel control value for the channels based on the provided masks */
...
void IfxGtm_Atom_Agc_enableChannelsOutput(Ifx_GTM_ATOM_AGC *agc, uint16 enableMask, uint16 disableMask, boolean immediate)
{
    ...
    /* Build the enable channel output control value for the channels based on the provided masks */
...
void IfxGtm_Atom_Agc_resetChannels(Ifx_GTM_ATOM_AGC *agc, uint32 resetMask)
{
    ...
    /* Build the reset value for the channels based on the provided masks */
    for (i = 0; i < IFXGTM_ATOM_NUM_AGC_CHANNELS; i++)
    {
        if (resetMask & 0x1)
        {
    ...
    /* Update the global control register with the reset value */
...
void IfxGtm_Atom_Ch_clearOneNotification(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel)
{
    ...
    /* Clears the CCU1 Trigger condition interrupt */
    atomCh->IRQ.NOTIFY.B.CCU1TC = 1;
}
...
void IfxGtm_Atom_Ch_clearZeroNotification(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel)
{
    ...
    /* Clears the CCU0 Trigger condition interrupt */
    atomCh->IRQ.NOTIFY.B.CCU0TC = 1;
}
...
void IfxGtm_Atom_Ch_configurePwmMode(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_Cmu_Clk clock, Ifx_ActiveState activeState, IfxGtm_Atom_Ch_ResetEvent resetEvent, IfxGtm_Atom_Ch_OutputTrigger trigger)
{
    ...
    /* Update the CMU clock source */
    ctrl.B.CLK_SRC_SR = clock;

    /* Update the Reset source of CCU0 */
    ctrl.B.RST_CCU0   = resetEvent;
    /* Update the Signal Level */
    ctrl.B.SL         = (activeState == Ifx_ActiveState_high ? 1 : 0);
    /*Update the Trigger output selection */
...
float32 IfxGtm_Atom_Ch_getClockFrequency(Ifx_GTM *gtm, Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel)
{
    ...
    /* Get the clock source configuration */
...
IfxGtm_Cmu_Clk IfxGtm_Atom_Ch_getClockSource(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel)
{
    IfxGtm_Cmu_Clk   clock;
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);
    clock = (IfxGtm_Cmu_Clk)atomCh->CTRL.B.CLK_SRC_SR;
...
volatile Ifx_SRC_SRCR *IfxGtm_Atom_Ch_getSrcPointer(Ifx_GTM *gtm, IfxGtm_Atom atom, IfxGtm_Atom_Ch channel)
{
    ...
    return &MODULE_SRC.GTM_ATOM[atom][channel / 2];
}
...
boolean IfxGtm_Atom_Ch_isOneNotification(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel)
{
    boolean          result;
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);
    /* Read the status of the CCU1 Trigger condition interrupt */
    result = atomCh->IRQ.NOTIFY.B.CCU1TC != 0;
    return result;
}
...
boolean IfxGtm_Atom_Ch_isZeroNotification(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel)
{
    boolean          result;
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);
    /* Read the status of the CCU0 Trigger condition interrupt */
    result = atomCh->IRQ.NOTIFY.B.CCU0TC != 0;
    return result;
}
...
void IfxGtm_Atom_Ch_raiseInterruptOne(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel)
{
    ...
    /* Trigger ATOM CCU1 Trigger condition interrupt by software */
    atomCh->IRQ.FORCINT.B.TRG_CCU1TC = 1;
}
...
void IfxGtm_Atom_Ch_raiseInterruptZero(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel)
{
    ...
    /* Trigger ATOM CCU0 Trigger condition interrupt by software */
    atomCh->IRQ.FORCINT.B.TRG_CCU0TC = 1;
}
...
void IfxGtm_Atom_Ch_setClockSource(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_Cmu_Clk clock)
{
    ...
    atomCh->CTRL.B.CLK_SRC_SR = clock;
}
...
void IfxGtm_Atom_Ch_setCompare(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, uint32 compareZero, uint32 compareOne)
{
    ...
    /* Update Compare 0 and Compare 1 values */
...
void IfxGtm_Atom_Ch_setNotification(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_IrqMode mode, boolean interruptOnCompareZero, boolean interruptOnCompareOne)
{
    ...
    en.U = atomCh->IRQ.EN.U;

    /* Disable all interrupts of the interrupt set to change mode */
    atomCh->IRQ.EN.U            = IFX_ZEROS;
    atomCh->IRQ.MODE.B.IRQ_MODE = mode;
    /* Set the values back */
    atomCh->IRQ.EN.U            = en.U;

    /* Update the interrupt enable for CCU0 Trigger condition */
    en.B.CCU0TC_IRQ_EN          = interruptOnCompareZero ? 1 : 0;
    /* Update the interrupt enable for CCU1 Trigger condition */
...
void IfxGtm_Atom_Ch_setSomcControl(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_Atom_SomcControl control)
{
    ...
    atomCh->SOMC.B.ACB_4_3_2 = control;
}
...
void IfxGtm_Atom_Ch_setSomcSignalLevelControl(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_Atom_SomcSignalLevelControl ctrl)
{
    ...
    atomCh->SOMC.B.ACB_1_0 = ctrl;
}


void IfxGtm_Atom_Ch_setSomiAcb0Control(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_Atom_SomiAcb0Control outputCtrl)
{
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);

    atomCh->SOMI.B.ACB0 = outputCtrl;
}


void IfxGtm_Atom_Ch_setSompAruDataSelect(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_Atom_SompAruDataSelect dataSelect)
{
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);

    atomCh->SOMP.B.ADL = dataSelect;
}


void IfxGtm_Atom_Ch_setSompPcm(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, boolean enabled)
{
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);

    atomCh->SOMP.B.BITREV = enabled;
}


void IfxGtm_Atom_Ch_setSompSr0Trig(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, boolean enabled)
{
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);

    atomCh->SOMP.B.SR0_TRIG = enabled;
}


void IfxGtm_Atom_Ch_setSomsShiftDirection(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_Atom_SomsShiftDirection dir)
{
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);

    atomCh->SOMS.B.ACB0 = dir;
}


void IfxGtm_Atom_Ch_setSomsDoubleShiftOutput(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, boolean enabled)
{
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);

    atomCh->SOMS.B.DSO = enabled;
}


void IfxGtm_Atom_Ch_setSombSignalLevelControl(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_Atom_SombSignalLevelControl ctrl)
{
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);

    atomCh->SOMB.B.ACB_1_0 = ctrl;
}


void IfxGtm_Atom_Ch_setSombCompareStrategy(Ifx_GTM_ATOM *atom, IfxGtm_Atom_Ch channel, IfxGtm_Atom_SombCompareStrategy compare)
{
    Ifx_GTM_ATOM_CH *atomCh = IfxGtm_Atom_Ch_getChannelPointer(atom, channel);

    atomCh->SOMB.B.ACB_4_3_2 = compare;
}
