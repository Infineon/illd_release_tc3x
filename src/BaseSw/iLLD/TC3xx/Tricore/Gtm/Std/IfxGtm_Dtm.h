...
/** \brief DTM CTRL UPD_MODE value for self-reset: SHUTOFF_SYNC is set when PSU_SHUT_OFF=1 and cleared when PSU_SHUT_OFF=0
 */
#define IFXGTM_DTM_UPD_MODE_SELF_RESET    (0x3u)
...
/**
 * \brief Set shutoff update mode in DTM CTRL register.
 *
 * \param[inout] dtm  Pointer to the DTM object.
 * \param[in]    mode Shutoff update mode. Range: 0 to 7.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Dtm_setShutoffUpdateMode(Ifx_GTM_CDTM_DTM *dtm, uint8 mode);

/**
 * \brief Reset the shutoff signal so as to resume normal DTM output.
 * Writing to SHUT_OFF_RST clears internal signal SHUTOFF_SYNC.
 *
 * \param[inout] dtm Pointer to the DTM object.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Dtm_resetShutoffSignal(Ifx_GTM_CDTM_DTM *dtm);
...
IFX_INLINE void IfxGtm_Dtm_setShutoffUpdateMode(Ifx_GTM_CDTM_DTM *dtm, uint8 mode)
{
    dtm->CTRL.B.UPD_MODE = mode;
}


IFX_INLINE void IfxGtm_Dtm_resetShutoffSignal(Ifx_GTM_CDTM_DTM *dtm)
{
    dtm->CTRL.B.SHUT_OFF_RST = 1u;
}
