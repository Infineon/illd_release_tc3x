...
#ifndef DEVICE_TC38EVOX
/** \brief Offset for Ifx_GTM_CLS_SPE_PAT_Bits.IP1_PAT
 */
#define IFXGTM_SPE_CLS_PAT_IP1_PAT_OFF (5u)

/** \brief Offset for Ifx_GTM_CLS_SPE_PAT_Bits.IP0_PAT
 */
#define IFXGTM_SPE_CLS_PAT_IP0_PAT_OFF (1u)
#endif
...
#else
    uint32 pattern : 3;     /**< \brief Input pattern for SPE */
#endif
...
/**
 * \brief Clears all the interrupts for the specified SPE device.
 *
 * \param[inout] spe Pointer to the SPE device instance.
 *
 * \retval None
 */
...
/**
 * \brief Clears the specified interrupt flag for the SPE device.
 *
 * \param[inout] spe 	   Pointer to the SPE device instance.
 * \param[in]    interrupt The specific interrupt signal to be cleared. Range: \ref IfxGtm_Spe_InterruptSignal
 *
 * \retval None
 */
...
/**
 * \brief Enables or disables the SPE module based on the boolean input.
 *
 * \param[inout] spe 	 Pointer to the SPE device instance.
 * \param[in] 	 enabled Enable status for the SPE sub-module (TRUE to enable, FALSE to disable).
 *
 * \retval None
 */
...
/**
 * \brief Retrieves the actual input pattern in word format for the specified SPE device.
 *
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval uint32 The actual input pattern as a 32-bit word.
 */
IFX_INLINE uint32 IfxGtm_Spe_getActualInputPat(Ifx_GTM_SPE *spe);

/**
 * \brief Retrieves the current backward pattern pointer from the SPE module.
 *
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval uint32 The current backward pattern pointer value.
 */
IFX_INLINE uint32 IfxGtm_Spe_getBwdPatternPtr(Ifx_GTM_SPE *spe);

/**
 * \brief Retrieves the current control command value used to select the pattern pointer which selects OUT_PAT.
 *
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval IfxGtm_Spe_ControlCommand The current control command selection. Range: \ref IfxGtm_Spe_ControlCommand
 */
IFX_INLINE IfxGtm_Spe_ControlCommand IfxGtm_Spe_getControlCommand(Ifx_GTM_SPE *spe);

/**
 * \brief Retrieves the actual rotation direction of the input signals for the specified SPE device.
 *
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval IfxGtm_Spe_Direction The actual rotation direction of the input signals. Range: \ref IfxGtm_Spe_Direction
 */
IFX_INLINE IfxGtm_Spe_Direction IfxGtm_Spe_getDir(Ifx_GTM_SPE *spe);

/**
 * \brief Retrieves the new input pattern in word format from the SPE device.
 *
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval uint32 The new input pattern in word format.
 */
IFX_INLINE uint32 IfxGtm_Spe_getNewInputPat(Ifx_GTM_SPE *spe);

/**
 * \brief Gets the output control register (OUT_CTRL) as a 32-bit word to read the current output pattern.
 *
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval uint32 The content of the output control register as a 32-bit value, reflecting the current output pattern.
 */
IFX_INLINE uint32 IfxGtm_Spe_getOutputCtrl(Ifx_GTM_SPE *spe);

/**
 * \brief Retrieves the output pattern for the specified index as a whole word.
 *
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \param[in] patIndex The index of the pattern to retrieve. Range: \ref IfxGtm_Spe_PatternIndex
 *
 * \retval uint32 The output pattern as a 32-bit word for the specified index.
 */
IFX_INLINE uint32 IfxGtm_Spe_getOutputPattern(Ifx_GTM_SPE *spe, IfxGtm_Spe_PatternIndex patIndex);

/**
 * \brief Returns the pattern pointer indicating the currently pointed SPE_OUT_PAT register.
 *
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval uint32 The current pattern pointer value.
 */
IFX_INLINE uint32 IfxGtm_Spe_getPatternPtr(Ifx_GTM_SPE *spe);

/**
 * \brief Retrieves the previous rotation direction of the SPE module.
 *
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval IfxGtm_Spe_Direction The previous direction of rotation. Range: \ref IfxGtm_Spe_Direction
 */
 ...
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval uint32 The previous input pattern value.
 */
 ...
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval uint32 Step counter value
 */
IFX_INLINE uint32 IfxGtm_Spe_getRevCounter(Ifx_GTM_SPE *spe);

/**
 * \brief Get the pointer to the Interrupt Request Node of the specified SPE object.
 *
 * \param[in] spe Spe Index Number. Range: \ref IfxGtm_Spe
 *
 * \retval Ifx_SRC_SRCR* Pointer to the SRC register of the specified SPE object.
 */
 ...
 *
 * \param[inout] spe 	  Pointer to the SPE device instance.
 * \param[in]    timInput The TIM input signal to be selected. Range: \ref IfxGtm_Spe_TimInput
 *
 * \retval None
 */
...
/**
 * \brief Sets the Backward pattern pointer to the CTRL_STAT2 register. This is used when the control command is set to backwards in the CTRL_CMD register.
 *
 * \param[inout] spe 	  Pointer to the SPE device instance.
 * \param[in] 	 patIndex Pattern index number. Range: \ref IfxGtm_Spe_PatternIndex
 *
 * \retval None
 */
...
/**
 * \brief Configures the CTRL_CMD field to select which pattern pointer is used to select OUT_PAT.
 *
 * \param[inout] spe Pointer to the SPE device instance.
 * \param[in] cmd  The control command selection of type IfxGtm_Spe_ControlCommand. Range: \ref IfxGtm_Spe_ControlCommand
 *
 * \retval None
 */
...
/**
 * \brief Sets the Fast Shutoff Mode for the SPE module.
 * 
 * \param[inout] spe     Pointer to the SPE device instance.
 * \param[in]    enabled Boolean flag to enable or disable Fast Shutoff.
...
/**
 * \brief Configures the interrupt mode for the SPE submodule.
 *
 * \param[inout] spe  Pointer to the SPE device instance.
 * \param[in]    mode The interrupt mode to be set for the SPE submodule. Range: \ref IfxGtm_Spe_InterruptMode
 *
 * \retval None
 */
...
/**
 * \brief Configures the output control (out_ctrl) register of the SPE module with the specified output pattern.
 *
 * \param[inout] spe    Pointer to the SPE device instance.
 * \param[in]    patPtr Pointer to the output pattern to be applied.
 *
 * \retval None
 */
 ...
 *
 * \param[inout] spe   Pointer to the SPE device instance.
 * \param[in]    count The new value to be set for the step counter. Range: 0 to 0x00FFFFFF.
 *
 * \retval None
 */
...
/**
 * \brief Disables the specified interrupt signal for the given SPE device.
 *
 * \param[inout] spe       Pointer to the SPE device instance.
 * \param[in]    interrupt The interrupt signal to disable. Range: \ref IfxGtm_Spe_InterruptSignal
 *
 * \retval None
 */
...
/**
 * \brief Enables or disables the error interrupt of the specified type for the SPE device.
 *
 * \param[inout] spe       Pointer to the SPE device instance.
 * \param[in]    interrupt The interrupt signal to enable or disable. Range: \ref IfxGtm_Spe_InterruptSignal
...
/**
 * \brief Enables or disables the input to the SPE from the selected input signal source.
 *
 * \param[inout] spe 	     Pointer to the SPE device instance.
 * \param[in] 	 inputSignal Input signal selector. Range: \ref IfxGtm_Spe_InputSignal
 * \param[in] 	 enabled   	 Boolean to enable (true) or disable (false) the input signal.
 *
 * \retval None
 */
...
/**
 * \brief Enables the specified SPE interrupt for the given SPE device.
 *
 * \param[inout] spe 	   Pointer to the SPE device instance.
 * \param[in] 	 interrupt The SPE interrupt signal to enable. Range: \ref IfxGtm_Spe_InterruptSignal
 *
 * \retval None
 */
...
/**
 * \brief Retrieves the fast shutoff level for a specified SPE output channel.
 *
 * \param[in] spe 	  Pointer to the SPE device instance.
 * \param[in] channel Output channel selection. Range: \ref IfxGtm_Spe_OutChannel
 *
 * \retval uint32 The fast shutoff level value for the specified channel.
 */
IFX_EXTERN uint32 IfxGtm_Spe_getFastShutoffLevel(Ifx_GTM_SPE *spe, IfxGtm_Spe_OutChannel channel);

/**
 * \brief Checks and returns the interrupt status of a specified interrupt signal for the SPE module.
 *
 * \param[in] spe 		Pointer to the SPE device instance.
 * \param[in] interrupt The interrupt signal to check. Range: \ref IfxGtm_Spe_InterruptSignal
 *
 * \retval TRUE If the specified interrupt is active.
 * 	       FALSE If the specified interrupt is not active.
 */
 ...
 * \param[in] spe Pointer to the SPE device instance.
 *
 * \retval uint32 Step counter compare value.
 */
IFX_EXTERN uint32 IfxGtm_Spe_getRevCompare(Ifx_GTM_SPE *spe);

/**
 * \brief Selects the trigger source for the SPE control output.
 *
 * \param[inout] spe 	 Pointer to the SPE device instance.
 * \param[in]    trigger The trigger source to be selected. Range: \ref IfxGtm_Spe_TriggerSource
 *
 * \retval None
 */
...
/**
 * \brief Configures all the specified input patterns into the SPE_PAT register
 * 
 * \param[inout] spe 	Pointer to the SPE device instance.
 * \param[in] 	 patPtr Pointer to the input pattern specified.
 * 
 * \retval None
 */
 ...
 * \param[inout] spe 	  Pointer to the SPE device instance.
 * \param[in] 	 patPtr   Pointer to the input pattern specified.
 * \param[in] 	 patIndex The index of the pattern specified. 
 *                     	  Range: \ref IfxGtm_Spe_PatternIndex
 *
 * \retval None
 */
 ...
 *
 * \param[inout] spe 	   Pointer to the SPE device instance.
 * \param[in]    interrupt Interrupt signal to be forced. Range: \ref IfxGtm_Spe_InterruptSignal
 *
 * \retval None
 */
 ...
 * \param[in] patIndex  The index of the pattern specified. Range: \ref IfxGtm_Spe_PatternIndex
 *
 * \retval None
 */
...
/**
 * \brief Configures the step compare count for the specified SPE module.
 *
 * \param[inout] spe   Pointer to the SPE device instance.
 * \param[in]    count Step counter value to be set. Range: 0 to 0x00FFFFFF.
 *
 * \retval None
 */
