...
{
	/* Disable the specified SPE interrupt signal */
...
void IfxGtm_Spe_enableInput(Ifx_GTM_SPE *spe, IfxGtm_Spe_InputSignal inputSignal, boolean enabled)
{
    ...
    else
    {
    	...
    	spe->CTRL_STAT.U &= ((uint32)enabled << ((uint32)inputSignal + IFX_GTM_SPE_CTRL_STAT_SIE0_OFF));
#else
        spe->CTRL_STAT.U &= ~(IFX_GTM_SPE_CTRL_STAT_SIE0_MSK << ((uint32)inputSignal + IFX_GTM_SPE_CTRL_STAT_SIE0_OFF));
#endif
    }
}
...
{
	/* Enable the specified SPE interrupt signal */
    uint32 irqen_temp = spe->IRQ.EN.U;
    irqen_temp    = (irqen_temp | ((uint32)1 << interrupt));
    spe->IRQ.EN.U = irqen_temp;
}
...
{
	/* Select the trigger source for the SPE control output */
...
{
	/* Update input pattern for all pattern index */
...
{
	/* Update input pattern for the specified pattern index */
    ...
    spe->PAT.U  = spepat_temp;
#else
    spe->PAT.U |= ((uint32)(pattern & 0xFu) << ((uint32)patIndex * (IFXGTM_SPE_CLS_PAT_IP1_PAT_OFF - IFXGTM_SPE_CLS_PAT_IP0_PAT_OFF)));
#endif
}
...
{
	/* Trigger the interrupt of specified type by software */
...
{
	/* Update the SPE output control value */
...
{
	/* Update the Input signal revolution counter compare value */
