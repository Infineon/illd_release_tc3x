 ...
 *
 * \param[in] driver Pointer to the TOM Timer interface Handle.
 *
 ...
 * \param[in] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval uint32* Pointer to the trigger channel register.
 */
 ...
 *
 * \param[inout] driver Pointer to the TOM Timer interface Handle.
 * \param[in]    config Pointer to the configuration structure containing initialization parameters for the TOM Timer.
 *
 * \retval TRUE If the timer object was successfully initialized.
 *         FALSE If the initialization failed.
 */
IFX_EXTERN boolean IfxGtm_Tom_Timer_init(IfxGtm_Tom_Timer *driver, const IfxGtm_Tom_Timer_Config *config);

/**
 * \brief Initializes the configuration structure for the GTM TOM Timer to default values.
 *
...
/**
 * \brief Returns the timer event and clears the timer interrupt request (IRQ) for the specified TOM Timer driver.
 *
 * \param[inout] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval TRUE If the If the timer event occurred. FALSE, otherwise.
 */
IFX_EXTERN boolean IfxGtm_Tom_Timer_acknowledgeTimerIrq(IfxGtm_Tom_Timer *driver);

/**
 * \brief Returns the trigger event and clears the trigger interrupt for the TOM Timer.
 *
 * \param[inout] driver Pointer to the TOM Timer interface Handle.
 *
...
/**
 * \brief Adds a specific channel to the channel mask of the TOM Timer.
 ...
 * 
 * \param[inout] driver  Pointer to the TOM Timer interface Handle.
 * \param[in]    channel The channel to be added to the mask. Range: \ref IfxGtm_Tom_Ch
 * 
 * \retval None
 */
...
/**
 * \brief Enables the transfer of shadow registers for the TOM Timer.
 *
 * \param[inout] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval None
...
/**
 * \brief Disables the update of the Timer, specifically stopping the transfer of shadow registers.
 *
 * \param[inout] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval None
 ...
 * \param[in] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval float32 The current frequency in Hz.
 */
IFX_EXTERN float32 IfxGtm_Tom_Timer_getFrequency(IfxGtm_Tom_Timer *driver);

/**
 * \brief Returns the input frequency in Hz.
 *
 * \param[in] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval float32 The input frequency in Hz.
 */
IFX_EXTERN float32 IfxGtm_Tom_Timer_getInputFrequency(IfxGtm_Tom_Timer *driver);

/**
 * \brief Retrieves the period of the timer.
 *
 * \param[in] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval Ifx_TimerValue The period of the timer in timer ticks.
 */
 ...
 * \param[in] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval float32 The resolution value.
 */
IFX_EXTERN float32 IfxGtm_Tom_Timer_getResolution(IfxGtm_Tom_Timer *driver);

/**
 * \brief Retrieves the trigger point.
 *
 * \param[in] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval Ifx_TimerValue The trigger point value.
 */
 ...
 *
 * \param[inout] driver    Pointer to the TOM Timer interface Handle.
 * \param[in]    frequency The desired frequency to be set for the timer. Range: Must be a positive value (frequency > 0).
 *
 * \retval TRUE If the frequency was successfully set.
 * 		   FALSE If the frequency setting failed.
 */
 ...
 *
 * \param[inout] driver Pointer to the TOM Timer interface Handle.
 * \param[in]    period The period value to be set for the timer, specified in timer ticks.             
 *
 * \retval TRUE If the period was successfully set.
 * 		   FALSE If the period setting failed.
 */
IFX_EXTERN boolean IfxGtm_Tom_Timer_setPeriod(IfxGtm_Tom_Timer *driver, Ifx_TimerValue period);

/**
 * \brief Configures the single shot mode for the TOM timer.
 * 
 * \param[inout] driver  Pointer to the TOM Timer interface Handle.
 * \param[in] 	 enabled Boolean flag to enable or disable single shot mode. If TRUE, the timer will operate in single shot mode.              	   
 *
 * \retval None
 */
...
/**
 * \brief Sets the trigger point for the TOM timer.
 *
 * \param[inout] driver       Pointer to the TOM Timer interface Handle.
...
/**
 * \brief Initializes the standard interface timer with the provided driver configuration.
 *
 * \param[inout] stdif Standard interface timer object that will be initialized by the function.
 * \param[in] driver Pointer to the interface driver to be used by the standard interface. Must be initialised separately
 *
...
/**
 * \brief Updates the input frequency for the TOM timer interface
 *
 * \param[inout] driver Pointer to the TOM Timer interface Handle.
 *
 * \retval None
