 ...
 *
 * \param[inout] driver Pointer to GTM TOM PWM driver.
...
/**
 * \brief Gets the dead time tick value.
 *
 * \param[in] driver Pointer to GTM TOM PWM driver.
 *
 * \retval None
 */
...
boolean IfxGtm_Tom_Dtm_PwmHl_init(IfxGtm_TomDtm_PwmHl *driver, const IfxGtm_Tom_Dtm_PwmHl_Config *config)
{
    ...
    for (channelIndex = 0; channelIndex < config->base.channelCount; channelIndex++)
    {
        ...
        driver->dtm[channelIndex] = IfxGtm_Dtm_getDtmPointer(timer->gtm, IfxGtm_Dtm_getCdtmIndexForTom(config->tom), IfxGtm_Dtm_getDtmIndexForTom(config->tom, config->ccx[channelIndex]->channel));
        ...
        if (driver->dtm[channelIndex] != NULL_PTR)
        {
            ...
            IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, IfxGtm_Dtm_getDtmChannelForTom(config->ccx[channelIndex]->channel) == IfxGtm_Dtm_getDtmChannelForTom(config->coutx[channelIndex]->channel));

            IfxGtm_Dtm_setClockSource(driver->dtm[channelIndex], config->deadTimeClock);
            driver->dtmChannel[channelIndex] = IfxGtm_Dtm_getDtmChannelForTom(config->ccx[channelIndex]->channel);
    ...
    {
        driver->dtmClockFreq = IfxGtm_Dtm_getClockFrequency(timer->gtm, IfxGtm_Dtm_getCdtmIndexForTom(config->tom), IfxGtm_Dtm_getDtmIndexForTom(config->tom, config->ccx[0]->channel));
...
void IfxGtm_Tom_Dtm_PwmHl_setOnTime(IfxGtm_TomDtm_PwmHl *driver, Ifx_TimerValue *tOn)
{
...
IFX_STATIC void IfxGtm_Tom_Dtm_PwmHl_updateCenterAligned(IfxGtm_TomDtm_PwmHl *driver, Ifx_TimerValue *tOn)
{
    ...
    for (channelIndex = 0; channelIndex < driver->base.channelCount; channelIndex++)
    {
        ...
        if (x == 0)
        {   /* 0% duty cycle */
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->ccx[channelIndex], (uint16)(period + 1), (uint16)(period + 1));
        }
        else
        {                           /* x% duty cycle and 100 % duty cycle  */
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->ccx[channelIndex], (uint16)(period - x) / 2, (uint16)(period + x) / 2);
        }
    }
}
...
IFX_STATIC void IfxGtm_Tom_Dtm_PwmHl_updateEdgeAligned(IfxGtm_TomDtm_PwmHl *driver, Ifx_TimerValue *tOn)
{
    ...
    for (channelIndex = 0; channelIndex < driver->base.channelCount; channelIndex++)
    {
        ...
        if (x == period)   /* 100% duty cycle */
        {
            cm0 = 0;
            cm1 = period + 1;
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->ccx[channelIndex], (uint16)cm0, (uint16)cm1);
        }
        else if (x == 0)
        {
            cm0 = period + 1;
    ...
    for (channelIndex = 0; channelIndex < driver->base.channelCount; channelIndex++)
    {
        ...
        {   /* 100% duty cycle */
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->ccx[channelIndex],
				(uint16)(period + 1) /* No compare event */,
