 ...
 *
 * \param[in] activeState The current active state to be inverted.Range: \ref Ifx_ActiveState
 *
 * \retval Ifx_ActiveState The inverted active state. Range: \ref Ifx_ActiveState
 */
...
boolean IfxGtm_Tom_PwmHl_init(IfxGtm_Tom_PwmHl *driver, const IfxGtm_Tom_PwmHl_Config *config)
{
    ...
    /* Initialize the dead time for the PWM signal */
    IfxGtm_Tom_PwmHl_setDeadtime(driver, config->base.deadtime);
    /* Initialize the minimum pulse width for the PWM signal */
...
void IfxGtm_Tom_PwmHl_initConfig(IfxGtm_Tom_PwmHl_Config *config)
{
	...
	/* Initialize the PWM HL configuration with default values */
    ...
    for (channelIndex = 0; channelIndex < driver->base.channelCount; channelIndex++)
    {
        ...
        {                       /* 100% duty cycle */
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->ccxTemp[channelIndex],
                ...
                (uint16)(2 /* 1st compare event (issue: expected to be 1) */ + deadtime));
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->coutxTemp[channelIndex],
                (uint16)period + 2 /* No compare event, issues has been seen with +1 */,
				(uint16)2 /* 1st compare event (issue: expected to be 1) */);
        }
        else if (x == 0)
        {
            cm0 = 1;
            ...
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->coutxTemp[channelIndex], (uint16)(cm0 + deadtime), (uint16)cm1);
        }
        else
        {                           /* x% duty cycle */
            cm1 = (period - x) / 2; /* CM1 */
            cm0 = (period + x) / 2; /* CM0 */
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->ccxTemp[channelIndex], (uint16)cm0, (uint16)(cm1 + deadtime));
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->coutxTemp[channelIndex], (uint16)(cm0 + deadtime), (uint16)cm1);
        }
    }
}
...
IFX_STATIC void IfxGtm_Tom_PwmHl_updateOff(IfxGtm_Tom_PwmHl *driver, Ifx_TimerValue *tOn)
{
    ...
    {
        IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->ccxTemp[channelIndex],
            2 /* 1 will keep the previous level */, (uint16)(period + 2));
        IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->coutxTemp[channelIndex], (uint16)(period + 1), 2);
    }
}
...
IFX_STATIC void IfxGtm_Tom_PwmHl_updatePulse(IfxGtm_Tom_PwmHl *driver, float32 *tOn, float32 *offset)
{
    ...
    for (channelIndex = 0; channelIndex < driver->base.channelCount; channelIndex++)
    {
        ...
        else if (x == 0)
        {
            cm0 = 1;
            cm1 = period + 2;
            IfxGtm_Tom_Ch_setCompareShadow(driver->tom, driver->coutxTemp[channelIndex], (uint16)cm0, (uint16)cm1);
        }
        else
        {                /* x% duty cycle */
            cm1 = 2 + o; /* CM1, set to 2 due to a GTM issue. should be 1 according to spec */
