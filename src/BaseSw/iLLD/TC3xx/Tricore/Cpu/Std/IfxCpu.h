 ...
 *
 *
 * \defgroup IfxLld_Cpu_Usage How to use the CPU Core?
 * \ingroup IfxLld_Cpu_Std
 *
 * Following section describes how to integrate the CPU API's into the application framework.
 *
 * \section IfxLld_Cpu_Preparation Preparation
 * \subsection IfxLld_Cpu_Include Include Files
 *
 ...
 * \endcode
 *
 * \subsection IfxLld_Cpu_Application API Usage Examples
 *
 ...
 * {
 * 		// critical section
 * 		IfxCpu_releaseMutex(&resourceLock);   // Mutex released
 * }
 * \endcode
 *
 * Set and Reset SpinLock
 *
 * \code
 ...
 * {
 * 		// critical section
 *      IfxCpu_resetSpinLock(&resourceLock); // Reset spinlock
 * }
 * \endcode
 *
 ...
 *
 * \code
 * // Disables, Reset and Start counters in normal mode
 ...
 * \endcode
 *
 * Disable interrupts for all other CPU except Master(Cpu0).
 *
 * \code
 * IfxCpu_disableInterruptsAllExceptMaster(IfxCpu_ResourceCpu_0);
 * \endcode
 *
 * Get the CPU index
 *
 ...
 * cpuIndex = IfxCpu_getIndex(&MODULE_CPU0);
 * \endcode
 *
 * Get a random Value
 *
 * \code
 * uint32 randomVal;
 * randomVal = IfxCpu_getRandomValue(&randomNum);
 * \endcode
 *
 * Get a random Value within range
 *
 * \code
 * uint32 randomValinRange;
 * randomValinRange = IfxCpu_getRandomValueWithinRange(&randomNum,0,0xffffffff);
 * \endcode
 *
 * Get the CPU Core Mode
 *
 * \code
 * IfxCpu_CoreMode coreMode;
 * coreMode = IfxCpu_getCoreMode(&MODULE_CPU0);
 * \endcode
 *
 * Set all CPU idle except master CPU
 *
 * \code
 * IfxCpu_setAllIdleExceptMasterCpu(IfxCpu_ResourceCpu_0);
 * \endcode
 *
 * Enable and Disable the overlay block for CPU
 *
 * \code
 * // Enable the overlay block for CPU0
 * IfxCpu_enableOverlayBlock(IfxCpu_Index_0, 1U, IfxCpu_OverlayMemorySelect_core1DsprPspr, IfxCpu_OverlayAddressMask_32byte, 0xB0000000, 0x90000000);
 *
 * // Disable the overlay block for CPU0
 * IfxCpu_disableOverlayBlock(IfxCpu_Index_0, 1U);
 * \endcode
 *
 * Set the program counter
 *
 * \code
 * IfxCpu_setProgramCounter(&MODULE_CPU1,0xA0400100U);
 * IfxCpu_startCore(&MODULE_CPU1,0xA0400100U);
 * \endcode
 *
 * Wait event and Emit event
 *
 * Used to achieve synchronisation between all cores.
 *
 * \code
...
#endif /* #if defined(DEVICE_TC33XED) || defined(DEVICE_TC35X) || defined(DEVICE_TC37XED) || defined(DEVICE_TC39XB) */
#ifdef DEVICE_TC39XB
...
/**
 * \brief API to get the core ID of the CPU of the caller.
 *
 * Caution: Core id of the cpu's may not be continguous and shouldn't be used to index cpu.
 * Use \ref IfxCpu_getCoreIndex() to get CPU no.
 * 
 * \retval IfxCpu_Id The core ID of the CPU.
 * 					 Range: \ref IfxCpu_Id
 */
 ...
 *
 * \retval IfxCpu_ResourceCpu Resource index of the CPU.
 * 							  Range: \ref IfxCpu_ResourceCpu
 */
...
* 
* \param[inout] csaBegin Pointer to start of context save area, shall not be NULL pointer.
* \param[in]    csaEnd   Pointer to end of context save area, shall be higher address than csaBegin.
*
* \retval None
*/
...
/**
 * \brief Set or clear the safety task identifier (PSW.S) on the current CPU.
 *
 * \param[in] safetyId The boolean value determining whether to set or clear the safety task identifier.
 *                     Range: TRUE:  Sets the safety task identifier (PSW.S = 1).
 *                     		  FASLE: Clears the safety task identifier (PSW.S = 0).
 *
 * \retval None
 */
 ...
 *
 * \param[in] cpu Pointer to the CPU hardware module (register memory map).
 *
 * \retval IfxCpu_CoreMode The current mode of the CPU.
 ...
 *
 * \retval IfxCpu_ResourceCpu The resource index of the CPU.
 * 							  Range: \ref IfxCpu_ResourceCpu
 */
 ...
 *
 * \param[in] cpu  Pointer to the CPU HW module (register memory map).
 * \param[in] mode CPU mode to be set by this API.
 *         		   Range: \ref IfxCpu_CoreMode
 *
 ...
 * \param[in]    programCounter Program counter value to be set
 ...
 *
 * \param[in] coreIndex The index of the CPU core for which to retrieve the reset status.
 *
 * \retval IfxCpu_ResetStatus Returns the reason for the last kernel reset of the specified CPU core (no reset, reset by SMU, or reset by software).
 * 							  Range: \ref IfxCpu_ResetStatus
 */
 ...
 * 					  Range: If TRUE  The function enables interrupts for the current CPU.
 * 					  		 If FALSE The function does nothing and leaves interrupts disabled.
 * \retval None
 */
 ...
 *
 * \param[in] segmentNumberMask Mask where bitfield 0 represents segment 0 and bitfield 16 represent segment F.
 * \param[in] enable 		    TRUE:  To enable the cacheability for selected segment.
 * 							    FALSE: To disable the cacheability for selected segment .
 *
 * \retval None
 */
 ...
 *
 * \param[in] address The memory address to be checked.
 *
 * \retval boolean TRUE:  The address belongs to a segment that is cacheable, either in the flash or the LMU memory.
 * 				   FALSE: The address does not belong to a cacheable segment.
 */
 ...
 * 					 Range: TRUE:  Enable the data cache.
 ...
 * 				 	 Range: TRUE:  Enable the program cache.
 ...
 * used for this activity towards other CPUs.
 *
 * \param[in] enable Command to enable or disable the performance counter.
 ...
 *
 * \param[in] address The memory address of the performance counter to be updated.
 *                    Range: 0 to 0xFFFFFFFF
 * \param[in] count   The new value to be set for the performance counter.
 *                    Range: 0 to 0x7FFFFFFF
 *
 * \retval None
 */
 ...
 *
 * \param[in] lock Pointer to the mutex lock structure that is to be acquired.
 *
 ...
 *
 * \param[inout] lock Pointer to the mutex lock structure to be released.
 *
 * \retval None
 *
 * \code
 ...
 *
 * \param[inout] lock Pointer to the spin lock structure to be reset.
 * 
 * \retval None
 */
 ...
 *         FALSE: The lock could not be acquired within the specified timeout.
 *
 * \code
 *    IfxCpu_spinLock resourceLock;
 *    boolean flag = IfxCpu_setSpinLock(&resourceLock, 0xFFFF);
 ...
 *
 * \param[in] cpu 		   The CPU core number to operate on.
 *						   Range: \ref IfxCpu_ResourceCpu
 * \param[in] overlayBlock The identifier of the overlay block to disable.
 * 						   Range: 0 to 0xFFFF
 * \retval None
 */
 ...
 *
 * \param[in] cpu                 Core number to configure the overlay for.
 * 						          Range: \ref IfxCpu_ResourceCpu
 * \param[in] overlayBlock        Overlay block to enable.
 * 						          Range: 0 to 0xFFFF
 * \param[in] overlayMemorySelect Overlay Memory Select.
 * 							      Range: \ref IfxCpu_OverlayMemorySelect
 * \param[in] overlayAddressMask  Overlay Address Mask.
 ...
 *
 * \param[inout] seed Pointer to seed value. This seed is used to generate the random value 
 *                    and will be updated for the next call.
 *
...
/**
 * \brief Generates a random value within a specified range using a seed value.
 *
 * This function utilizes the provided seed to produce a random value between the
 * specified minimum and maximum values. The seed is updated to ensure different
 * results on subsequent calls.
 *
 * \param[inout] seed Pointer to the seed value. The seed will be updated for future random number generation.
...
/**
 * \brief This function waits till all the cores have set their corresponding bits in the event.
 *
 * This function along with \ref IfxCpu_emitEvent() are used to achieve the synchronisation between the configured cores. By default
 ...
 * Core 5: 6
 *
 * \param[in] event           Pointer to the synchronization event structure. This structure is used to track the synchronization state across cores.
 * \param[in] timeoutMilliSec Timeout value in milliseconds. The function will wait for this duration before returning with a timeout error.
 * 							  Range: 0 to 0xFFFFFFFF
 *
 * \retval TRUE:  Indicates that a timeout occurred while waiting for all cores to synchronize.
 *		   FALSE: Indicates that all cores successfully synchronized within the specified timeout.
 *
 ...
 *
 * \param[in] event Pointer to the synchronous event structure to be emitted.
 *
 * \retval None
 *
 ...
 *
 * \param[in] masterCpu Master CPU Index- All except this will be set to Idle Mode.
 * 						Range: \ref IfxCpu_ResourceCpu
 *
 * \retval None
 */
 ...
 *
 * \param[in] masterCpu Master CPU Index- All except this will have their interrupts disabled.
 * 						Range: \ref IfxCpu_ResourceCpu
 *
 * \retval None
 */
...
#endif /* #if !((defined(__cplusplus)) && (defined(__TASKING__))) */
