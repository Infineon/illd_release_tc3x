 ...
 *
 * \defgroup IfxLld_Convctrl_Usage How to use the Convctrl Interface driver?
 * \ingroup IfxLld_Convctrl
 * \{
 *
 * The Convctrl interface driver provides a default configuration for initializes the converter control module,
 * injects faults, and checks the resulting alarm flags. It verifies that the alarm flags are set and cleared
 * correctly in response to phase sync divider and counter faults.
 *
 ...
 *
 * \section IfxLld_Convctrl Preparation Preparation
 * \subsection IfxLld_Convctrl_Include Include Files
 *
 * Include following header file into your C code:
 *
 * \code
 *      #include <Convctrl/Std/IfxConvctrl.h>
 * \endcode
 *
 * \subsection IfxLld_Convctrl_Module Module initialisation
 *
 * Initialize the Convctrl with following code:
 * \code
 *  // initialize module
 *  IfxConvctrl_initializeModule(&MODULE_CONVCTRL, IfxConvctrl_PhaseSynchronizerDivider_6);
 * \endcode
 *
 * Resets the Convctrl with following code:
 * \code
 *  // Reset module
 *  IfxConvctrl_resetModule(&MODULE_CONVCTRL);
 * \endcode
 *
 * \}
 *
...
/**
 * \brief Clears the Converter Controller alarm flag and disables all module-level alarms.
 *
 * \param[inout] convctrl Pointer to the Converter Controller SFRs structure.
 *
 * \retval None
 */
...
/**
 * \brief Enables access to all CONVCTRL registers for configuration.
 *
 * \param[inout] convctrl Pointer to the Converter Controller SFRs structure.
 *
 * \retval None
...
/**
 * \brief Injects a fault into the Down Counter of the Converter Controller.
 *
 * \param[inout] convctrl Pointer to the Converter Controller SFRs structure.
 *
 * \retval None
...
/**
 * \brief Injects a fault into the Phase Synchronizer Divider (PHSDIV) of the converter controller.
 *
 * \param[inout] convctrl Pointer to the Converter Controller SFRs structure.
 *
 * \retval None
...
/**
 * \brief Checks if an error has been detected in the converter controller.
 *
 * \param[in] convctrl Pointer to the Converter Controller SFRs structure.
 *
 * \retval TRUE If safety problem has been detected.
 *         FALSE If no error indicated.
 */
IFX_INLINE boolean IfxConvctrl_isErrorDetected(Ifx_CONVERTER *convctrl);

/**
 * \brief Checks if the Converter Controller module is suspended.
 *
 * \param[in] convctrl Pointer to the Converter Controller SFRs structure.
 *
...
/**
 * \brief Locks the configuration access for the Converter Controller.
 *
 * \param[inout] convctrl Pointer to the Converter Controller SFRs structure.
 *
 * \retval None
...
/**
 * \brief Sets the Phase Synchronizer Divider value for the Converter Controller.
 *
 * \param[inout] convctrl     Pointer to the Converter Controller SFRs structure.
 * \param[in]    dividerValue The Phase Synchronizer divider value to be set.
 *                            Range: \ref IfxConvctrl_PhaseSynchronizerDivider.
 *
 * \retval None
 */
...
/**
 * \brief Configures the suspend mode of the converter controller.
 *
 * \param[inout] convctrl Pointer to the Converter Controller SFRs structure.
 * \param[in]    mode     Suspend mode to be configured. The mode determines how the converter reacts to the suspend signal.
 *                        Range: \ref IfxConvctrl_SuspendMode.
 *
 * \retval None
 */
...
/**
 * \brief Resets the Converter Controller module to its default state.
 *
 * \param[inout] convctrl Pointer to the Converter Controller SFRs structure.
 *
 * \retval None
...
/**
 * \brief Initializes the converter module with the specified phase synchronizer divider.
 *
 * \param[inout] convctrl       Pointer to the Converter Controller SFRs structure.
 * \param[in]    divisionFactor Prescaling factor between the peripheral clock and the phase synchronization signal.
 *                              Range: \ref IfxConvctrl_PhaseSynchronizerDivider.
 *
 * \retval None
 */
