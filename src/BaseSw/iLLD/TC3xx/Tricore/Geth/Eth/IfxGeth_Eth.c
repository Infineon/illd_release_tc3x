...
{
    uint32 channelIndex, txChannelIndex, rxChannelIndex;
    ...
    /* Configure the address aligned beats mode from the configuration table */
    geth->gethSFR->DMA_SYSBUS_MODE.B.AAL = dmaConfig->addressAlignedBeatsEnabled;
    /* Configure the Fixed Burst mode from the configuration table */
    geth->gethSFR->DMA_SYSBUS_MODE.B.FB = dmaConfig->fixedBurstEnabled;
    /* Configure the mixed burst mode from the configuration table*/
    geth->gethSFR->DMA_SYSBUS_MODE.B.MB = dmaConfig->mixedBurstEnabled;

    /* Re-initialize the DMA Channels */
    ...
    {
        /* Check if the DMA channel need to be enabled */
        if(dmaConfig->txChannel[txChannelIndex].channelEnable == TRUE)
        {
            /* Enable the Tx DMA channels */
            IfxGeth_Eth_initTxDmaChannel(geth, txChannelIndex, &dmaConfig->txChannel[txChannelIndex]);
		}
	}
    for (rxChannelIndex = 0; rxChannelIndex < dmaConfig->numOfRxChannels; rxChannelIndex++)
    {
        /* check if the DMA channel needs to be enabled */
        if(dmaConfig->rxChannel[rxChannelIndex].channelEnable == TRUE)
        {
            /* Enable the Rx DMA channels  */
            IfxGeth_Eth_initRxDmaChannel(geth, rxChannelIndex, &dmaConfig->rxChannel[rxChannelIndex]);
        }
    }
    /* Initialize the selected interrupts */
    for (channelIndex = 0; channelIndex < numOfChannels; channelIndex++)
    {
        
        /* Clear all the interrupt flags for the DMA channel */
        geth->gethSFR->DMA_CH[(dmaConfig->txInterrupt[channelIndex].channelId)].STATUS.U = 0;
        /* Transmit interrupts */
        IfxGeth_Eth_initTxDmaInterrupts(geth, channelIndex, &dmaConfig->txInterrupt[channelIndex]);
        /* Receive interrupts */
        IfxGeth_Eth_initRxDmaInterrupts(geth, channelIndex, &dmaConfig->rxInterrupt[channelIndex]);
    }
}


void IfxGeth_Eth_initTxDmaChannel(IfxGeth_Eth *geth, uint32 txChannelIndex, const IfxGeth_Eth_TxChannelConfig *txChannelConfig)
{
    /* Set the burst length from the configuration table */
    geth->gethSFR->DMA_CH[txChannelConfig->channelId].TX_CONTROL.B.TXPBL = txChannelConfig->maxBurstLength;
    /* Enable the OSF mode */
    geth->gethSFR->DMA_CH[txChannelConfig->channelId].TX_CONTROL.B.OSF = txChannelConfig->enableOSF;
    /* Initialize the transmit descriptors */
    IfxGeth_Eth_initTransmitDescriptors(geth, txChannelConfig);
}


void IfxGeth_Eth_initRxDmaChannel(IfxGeth_Eth *geth, uint32 rxChannelIndex, const IfxGeth_Eth_RxChannelConfig *rxChannelConfig)
{
    /* Set the maximum burst length from the configuration table */
    geth->gethSFR->DMA_CH[rxChannelConfig->channelId].RX_CONTROL.B.RXPBL = rxChannelConfig->maxBurstLength;
    /* Initialize the receive descriptors */
    IfxGeth_Eth_initReceiveDescriptors(geth, rxChannelConfig);
}
...
void IfxGeth_Eth_initTxDmaInterrupts(IfxGeth_Eth *geth, uint32 channelIndex, const IfxGeth_Eth_DmaInterruptConfig *dmaInterruptConfig)
{
    ...
    {      
        ...
        uint32 value = (1 << IfxGeth_DmaInterruptFlag_transmitInterrupt);
        ...
        /* Set the service priority number,  set who should service the interrupt DMA/CPU, Clear previous service requests */
		IfxSrc_init(srcSFR, dmaInterruptConfig->provider, dmaInterruptConfig->priority);
        /* Enable the Service Request (Interrupt) */
...
void IfxGeth_Eth_initRxDmaInterrupts(IfxGeth_Eth *geth, uint32 channelIndex, const IfxGeth_Eth_DmaInterruptConfig *dmaInterruptConfig)
{
    ...
    {       
        /* Prepare the interrupt enable flag for the current DMA channel */
        uint32 value = (1 << IfxGeth_DmaInterruptFlag_receiveInterrupt);
        /* Write the DMA channel's Interrupt enable flag */
        geth->gethSFR->DMA_CH[dmaInterruptConfig->channelId].INTERRUPT_ENABLE.U |= value;

        volatile Ifx_SRC_SRCR *srcSFR;
        /* Get the address of the SRC register corresponding to the Rx DMA Channel */
        srcSFR = IfxGeth_getSrcPointer(geth->gethSFR, (IfxGeth_ServiceRequest)((uint32)IfxGeth_ServiceRequest_6 + (uint32)dmaInterruptConfig->channelId));
        /* Set the service priority number,  set who should service the interrupt DMA/CPU, Clear previous service requests */
        IfxSrc_init(srcSFR, dmaInterruptConfig->provider, dmaInterruptConfig->priority);
        /* Enable the Service Request (Interrupt) */
        IfxSrc_enable(srcSFR);
...
void IfxGeth_Eth_configureMTL(IfxGeth_Eth *geth, IfxGeth_Eth_MtlConfig *mtlConfig)
{
    ...
    /* Set the scheduling algorithm for the configured Tx Queues */
    if (mtlConfig->numOfTxQueues > 1)
    {
        geth->gethSFR->MTL_OPERATION_MODE.B.SCHALG = mtlConfig->txSchedulingAlgorithm;
    }
    /* Enable the service request */
    for (txQueueIndex = 0; txQueueIndex < mtlConfig->numOfTxQueues; txQueueIndex++)
    {

        if(mtlConfig->txQueue[txQueueIndex].queueEnable == TRUE)
        {
            IfxGeth_Eth_configureTxQueue(geth->gethSFR, txQueueIndex, &mtlConfig->txQueue[txQueueIndex]);
        }
    }

    /* Set the Arbitration algorithm for the configured Rx Queues */
    if (mtlConfig->numOfRxQueues > 1)
    {        
        geth->gethSFR->MTL_OPERATION_MODE.B.RAA = mtlConfig->rxArbitrationAlgorithm;
    }

    /* Configure the RX Queues */
    for (rxQueueIndex = 0; rxQueueIndex < mtlConfig->numOfRxQueues; rxQueueIndex++)
    {
        if(mtlConfig->rxQueue[rxQueueIndex].queueEnable == TRUE)
        {
            IfxGeth_Eth_configureRxQueue(geth->gethSFR, rxQueueIndex, &mtlConfig->rxQueue[rxQueueIndex]);
        }
    }
    /* Check if there is a non-zero priority configured */
    if (mtlConfig->interrupt.priority > 0)
    {
        volatile Ifx_SRC_SRCR *srcSFR;
        /* Get the address of the SRC register corresponding to the MTL Queue */
        srcSFR = IfxGeth_getSrcPointer(geth->gethSFR, mtlConfig->interrupt.serviceRequest);
        /* Configure the service priority number and type of service */
...
void IfxGeth_Eth_configureTxQueue(Ifx_GETH *gethSFR, uint32 txQueueIndex, const IfxGeth_Eth_TxQueueConfig *txQueueConfig)
{
    ...
    case IfxGeth_TxMtlQueue_0:
        /* Set Queue size */
        gethSFR->MTL_TXQ0.OPERATION_MODE.B.TQS = txQueueConfig->txQueueSize;
        /* Enable store and forward feature */
        gethSFR->MTL_TXQ0.OPERATION_MODE.B.TSF = txQueueConfig->storeAndForward;
        /* Enable Tx Queue */
        gethSFR->MTL_TXQ0.OPERATION_MODE.B.TXQEN = 2;
        /* Check if TX Queue Underflow needs to be enabled */
        /* Set up the interrupt flags for which interrupt should be generated */
        if (txQueueConfig->txQueueUnderflowInterruptEnabled)
        {
            gethSFR->MTL_Q0.INTERRUPT_CONTROL_STATUS.U = (gethSFR->MTL_Q0.INTERRUPT_CONTROL_STATUS.U & ~IFXGETH_QUEUE_INTERRUPT_MASK) | IFXGETH_TXQUEUE_INTERRUPT_VALUE;
        }

        break;
    case IfxGeth_TxMtlQueue_1:
        /* Set Queue size */
        gethSFR->MTL_TXQ1.OPERATION_MODE.B.TQS = txQueueConfig->txQueueSize;
        /* Enable store and forward feature */
        gethSFR->MTL_TXQ1.OPERATION_MODE.B.TSF = txQueueConfig->storeAndForward;
        /* Enable Tx Queue */
        gethSFR->MTL_TXQ1.OPERATION_MODE.B.TXQEN = 2;
        /* Set up the interrupt flags for which interrupt should be generated */
        if (txQueueConfig->txQueueUnderflowInterruptEnabled)
        {
            gethSFR->MTL_Q1.INTERRUPT_CONTROL_STATUS.U = (gethSFR->MTL_Q1.INTERRUPT_CONTROL_STATUS.U & ~IFXGETH_QUEUE_INTERRUPT_MASK) | IFXGETH_TXQUEUE_INTERRUPT_VALUE;
        }

        break;
    case IfxGeth_TxMtlQueue_2:
        /* Set Queue size */
        gethSFR->MTL_TXQ2.OPERATION_MODE.B.TQS = txQueueConfig->txQueueSize;
        /* Enable store and forward feature */
        gethSFR->MTL_TXQ2.OPERATION_MODE.B.TSF = txQueueConfig->storeAndForward;
        /* Enable Tx Queue */
        gethSFR->MTL_TXQ2.OPERATION_MODE.B.TXQEN = 2;
        /* Set up the interrupt flags for which interrupt should be generated */
        if (txQueueConfig->txQueueUnderflowInterruptEnabled)
        {
            gethSFR->MTL_Q2.INTERRUPT_CONTROL_STATUS.U = (gethSFR->MTL_Q2.INTERRUPT_CONTROL_STATUS.U & ~IFXGETH_QUEUE_INTERRUPT_MASK) | IFXGETH_TXQUEUE_INTERRUPT_VALUE;
        }

        break;
    case IfxGeth_TxMtlQueue_3:
        /* Set Queue size */
        gethSFR->MTL_TXQ3.OPERATION_MODE.B.TQS = txQueueConfig->txQueueSize;
        /* Enable store and forward feature */
        gethSFR->MTL_TXQ3.OPERATION_MODE.B.TSF = txQueueConfig->storeAndForward;
        /* Enable Tx Queue */
        gethSFR->MTL_TXQ3.OPERATION_MODE.B.TXQEN = 2;
        /* Set up the interrupt flags for which interrupt should be generated */
        if (txQueueConfig->txQueueUnderflowInterruptEnabled)
        {
            gethSFR->MTL_Q3.INTERRUPT_CONTROL_STATUS.U = (gethSFR->MTL_Q3.INTERRUPT_CONTROL_STATUS.U & ~IFXGETH_QUEUE_INTERRUPT_MASK) | IFXGETH_TXQUEUE_INTERRUPT_VALUE;
        }
        break;
    }
}
...
void IfxGeth_Eth_configureRxQueue(Ifx_GETH *gethSFR, uint32 rxQueueIndex, const IfxGeth_Eth_RxQueueConfig *rxQueueConfig)
{

    if (rxQueueConfig->daBasedDmaChannelEnabled != 0)
    {  
        gethSFR->MTL_RXQ_DMA_MAP0.U |= ((uint32)TRUE << (((IfxGeth_RxMtlQueue)rxQueueIndex * 8) + 4));
    }
    else
    {    
        gethSFR->MTL_RXQ_DMA_MAP0.U |= (rxQueueConfig->rxDmaChannelMap << ((IfxGeth_RxMtlQueue)rxQueueIndex * 8));
    }
    ...
    case IfxGeth_RxMtlQueue_0:
        /* Set the queue size */
        gethSFR->MTL_RXQ0.OPERATION_MODE.B.RQS = rxQueueConfig->rxQueueSize;
        /* Enable Rx store and forward feature */
        gethSFR->MTL_RXQ0.OPERATION_MODE.B.RSF = rxQueueConfig->storeAndForward;
        /* Enable Rx Forward Error packet feature */
        gethSFR->MTL_RXQ0.OPERATION_MODE.B.FEP = rxQueueConfig->forwardErrorPacket;
        /* Enable Rx Forward undersized good packet feature */
        gethSFR->MTL_RXQ0.OPERATION_MODE.B.FUP = rxQueueConfig->forwardUndersizedGoodPacket;
        /* Set up the interrupt flags for which interrupt should be generated */
        if (rxQueueConfig->rxQueueOverflowInterruptEnabled)
        {
            gethSFR->MTL_Q0.INTERRUPT_CONTROL_STATUS.U = (gethSFR->MTL_Q0.INTERRUPT_CONTROL_STATUS.U & ~IFXGETH_QUEUE_INTERRUPT_MASK) | IFXGETH_RXQUEUE_INTERRUPT_VALUE;
        }
        break;
    case IfxGeth_RxMtlQueue_1:
        /* Set the queue size */
        gethSFR->MTL_RXQ1.OPERATION_MODE.B.RQS = rxQueueConfig->rxQueueSize;
        /* Enable Rx store and forward feature */
        gethSFR->MTL_RXQ1.OPERATION_MODE.B.RSF = rxQueueConfig->storeAndForward;
        /* Enable Rx Forward Error packet feature */
        gethSFR->MTL_RXQ1.OPERATION_MODE.B.FEP = rxQueueConfig->forwardErrorPacket;
        /* Enable Rx Forward undersized good packet feature */
        gethSFR->MTL_RXQ1.OPERATION_MODE.B.FUP = rxQueueConfig->forwardUndersizedGoodPacket;
        /* Set up the interrupt flags for which interrupt should be generated */
        if (rxQueueConfig->rxQueueOverflowInterruptEnabled)
        {
            gethSFR->MTL_Q1.INTERRUPT_CONTROL_STATUS.U = (gethSFR->MTL_Q1.INTERRUPT_CONTROL_STATUS.U & ~IFXGETH_QUEUE_INTERRUPT_MASK) | IFXGETH_RXQUEUE_INTERRUPT_VALUE;
        }
        break;
    case IfxGeth_RxMtlQueue_2:
        /* Set the queue size */
        gethSFR->MTL_RXQ2.OPERATION_MODE.B.RQS = rxQueueConfig->rxQueueSize;
        /* Enable Rx store and forward feature */
        gethSFR->MTL_RXQ2.OPERATION_MODE.B.RSF = rxQueueConfig->storeAndForward;
        /* Enable Rx Forward Error packet feature */
        gethSFR->MTL_RXQ2.OPERATION_MODE.B.FEP = rxQueueConfig->forwardErrorPacket;
        /* Enable Rx Forward undersized good packet feature */
        gethSFR->MTL_RXQ2.OPERATION_MODE.B.FUP = rxQueueConfig->forwardUndersizedGoodPacket;
        /* Set up the interrupt flags for which interrupt should be generated */
        if (rxQueueConfig->rxQueueOverflowInterruptEnabled)
        {
            gethSFR->MTL_Q2.INTERRUPT_CONTROL_STATUS.U = (gethSFR->MTL_Q2.INTERRUPT_CONTROL_STATUS.U & ~IFXGETH_QUEUE_INTERRUPT_MASK) | IFXGETH_RXQUEUE_INTERRUPT_VALUE;
        }
        break;
    case IfxGeth_RxMtlQueue_3:
        /* Set the queue size */
        gethSFR->MTL_RXQ3.OPERATION_MODE.B.RQS = rxQueueConfig->rxQueueSize;
        /* Enable Rx store and forward feature */
        gethSFR->MTL_RXQ3.OPERATION_MODE.B.RSF = rxQueueConfig->storeAndForward;
        /* Enable Rx Forward Error packet feature */
        gethSFR->MTL_RXQ3.OPERATION_MODE.B.FEP = rxQueueConfig->forwardErrorPacket;
        /* Enable Rx Forward undersized good packet feature */
        gethSFR->MTL_RXQ3.OPERATION_MODE.B.FUP = rxQueueConfig->forwardUndersizedGoodPacket;
        /* Set up the interrupt flags for which interrupt should be generated */
        if (rxQueueConfig->rxQueueOverflowInterruptEnabled)
        {
            gethSFR->MTL_Q3.INTERRUPT_CONTROL_STATUS.U = (gethSFR->MTL_Q3.INTERRUPT_CONTROL_STATUS.U & ~IFXGETH_QUEUE_INTERRUPT_MASK) | IFXGETH_RXQUEUE_INTERRUPT_VALUE;
        }
        break;
    }
    /* Enable the Rx Queue */
    gethSFR->MAC_RXQ_CTRL0.U |= (2 << ((IfxGeth_RxMtlQueue)rxQueueIndex * 2));
}
            ...
            .txChannel                  = {
                {
                    ...
                    .maxBurstLength        = IfxGeth_DmaBurstLength_8,
...
void IfxGeth_Eth_initReceiveDescriptors(IfxGeth_Eth *geth, const IfxGeth_Eth_RxChannelConfig *config)
{
    int                       i;
    ...
    geth->gethSFR->DMA_CH[channelId].RXDESC_LIST_ADDRESS.U = (uint32)geth->rxChannel[channelId].rxDescrList->descr;;
    geth->gethSFR->DMA_CH[channelId].RXDESC_TAIL_POINTER.U = (uint32)descr;
    geth->gethSFR->DMA_CH[channelId].RXDESC_RING_LENGTH.U = (IFXGETH_MAX_RX_DESCRIPTORS - 1);
}


void IfxGeth_Eth_initTransmitDescriptors(IfxGeth_Eth *geth, const IfxGeth_Eth_TxChannelConfig *config)
{
    int                  i;
    ...
    geth->gethSFR->DMA_CH[channelId].TXDESC_RING_LENGTH.U = (IFXGETH_MAX_TX_DESCRIPTORS - 1);
}
