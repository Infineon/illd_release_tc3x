...
/**
 * \brief Prepares the module to receive the LIN header.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 *
 * \retval None
 */
...
/**
 * \brief Prepares the module to receive the LIN response.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 * \param[in]    length Number of bytes to be received.
 *                      Range: 1 to 8 (maximum LIN response size).
 *
 * \retval None
 */
...
/**
 * \brief Reads the LIN header.
 *
 * \param[in]  asclin Pointer to the LIN module handle.
 * \param[out] id     Pointer to store the received header ID byte.
 *                    Range: 0x00 to 0xFF.
 *
 * \retval None
 */
...
/**
 * \brief Ignores the latest LIN header.
 *
 * \param[inout]  asclin Pointer to the LIN module handle.
 *
 * \retval None
...
/**
 * \brief Receives the LIN response.
 *
 * \param[inout]  asclin Pointer to the LIN module handle.
 * \param[out]    data   Pointer to the buffer where the received data will be stored.
...
/**
 * \brief Sends the LIN header.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 * \param[in]    id     Pointer to the header ID byte to be sent.
 *                      Range: 0x00 to 0xFF.
 *
 * \retval None
 */
 ...
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 * \param[in]    config Pointer to the configuration structure containing initialization settings.
 *
...
/**
 * \brief Initializes the LIN configuration structure with default values.
 *
 * \param[out] config Pointer to the LIN configuration structure.
 *                    This structure will be filled with default values suitable for LIN communication.
...
/**
 * \brief Sends a LIN frame. This API must be used in interrupt mode. It can be used to
 * 1.Send Header
 * 2.Send both Header and Response
 * 3.Send Header and Receive Response
 *
 * This function sends a complete LIN frame, including the header and response,
 * or just the header, based on the provided PDU configuration.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 * \param[in]    pduptr Pointer to the LIN PDU configuration structure.
 *
 * \retval None
 */
...
/**
 * \brief Checks for received header flags and updates acknowledgment and error flags.
 *
 * \param[inout] asclin Pointer to the LIN module handle. Upon return, acknowledgmentFlags and errorFlagsStatus are updated in the LIN module handle.
 *
 * \retval None
 */
...
/**
 * \brief Checks for received response flags and updates the acknowledgment and error flags accordingly.
 *
 * \param[inout] asclin Pointer to the LIN module handle. Upon return, acknowledgmentFlags and errorFlagsStatus are updated in the LIN module handle.
 *
 * \retval None
...
/**
 * \brief Checks for transmitted header flags and updates the acknowledgment and error flags accordingly.
 *
 * \param[inout] asclin Pointer to the LIN module handle. Upon return, acknowledgmentFlags and errorFlagsStatus are updated in the LIN module handle.
 *
 * \retval None
...
/**
 * \brief Checks for transmitted response flags and updates the acknowledgment and error flags accordingly.
 *
 * \param[inout] asclin Pointer to the LIN module handle. Upon return, acknowledgmentFlags and errorFlagsStatus are updated in the LIN module handle.
 *
 * \retval None
...
/**
 * \brief Clears the flags status structures.This function clears the acknowledgment and error flags status in the LIN module handle.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 *
 * \retval None
...
/**
 * \brief Waits until receive header end acknowledgment or an error has been detected.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 *
 * \retval TRUE: if error, FALSE: is no error
 */
IFX_EXTERN boolean IfxAsclin_Lin_waitForReceivedHeader(IfxAsclin_Lin *asclin);

/**
 * \brief Waits until receive response end acknowledgment or an error has been detected.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 *
 * \retval TRUE: if error, FALSE: is no error
...
/**
 * \brief Waits until transmit header end acknowledgment or an error has been detected.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 *
 * \retval TRUE: if error, FALSE: is no error
...
/**
 * \brief Waits until transmit response end acknowledgment or an error has been detected.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 *
 * \retval TRUE: if error, FALSE: is no error
...
/**
 * \brief ISR for LIN error handling. Processes and clears LIN error flags, and updates error flags status based on error conditions.
 *
 * \param[inout] asclin Pointer to the LIN module handle. Upon return, errorFlagsStatus are updated in the LIN module handle.
 *
 * \retval None
 */
...
/**
 * \brief ISR for LIN receive handling.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 *
 * \retval None
...
/**
 * \brief ISR for LIN transmit handling.
 *
 * \param[inout] asclin Pointer to the LIN module handle.
 */
...
/**
 * \brief LIN Tx and Rx interrupts initialization function.
 *
 * \param[inout] asclinSFR         Pointer to the ASCLIN module's special function register (SFR).
 * \param[in]    interruptConfig   Pointer to the interrupt configuration structure.
 * \param[in]    interruptModeFlag Boolean flag to enable or disable interrupt mode.
 *
 * \retval None
 */
IFX_INLINE void IfxAsclin_Lin_initInterrupt(Ifx_ASCLIN *asclinSFR, const IfxAsclin_Lin_InterruptConfig *interruptConfig, boolean interruptModeFlag);

/**
 * \brief Initializes the Tx and Rx pins for the ASCLIN LIN module.
 *
 * \param[in] pins Pointer to the structure containing the RX and TX pin configuration details.
 *
 * \retval None
 */
IFX_INLINE void IfxAsclin_Lin_initPin(const IfxAsclin_Lin_Pins *pins);
