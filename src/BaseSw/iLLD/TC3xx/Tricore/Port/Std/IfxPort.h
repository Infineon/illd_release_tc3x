...
/**
 * \brief Enable the emergency stop function for a specified pin of a port.
 ...
 * \retval None
 * 
 * \note It is the User's responsibility to pass correct port, pin, and mode inputs.
 *
 */
...
/**
 * \brief Retrieves the memory address of the specified PORT module.
 *
 * \param[in] port The index of the PORT module to be accessed.
 *
 * \retval Ifx_P* The base address of the PORT module registers.
 *
 */
IFX_EXTERN Ifx_P *IfxPort_getAddress(IfxPort_Index port);

/**
 * \brief Returns the port index within the IfxModule_IndexMap (defined in IfxPort_cfg.c)
 *
 * \param[in] port Pointer to the port for which the index number in IfxModule_IndexMap should be retrieved.
 *
 * \retval IfxPort_Index Port index of IfxModule_IndexMap. Returns -1 if the port index is unknown. Range: \ref IfxPort_Index.
 *
 */
