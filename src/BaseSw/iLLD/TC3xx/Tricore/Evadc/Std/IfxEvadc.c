...
void IfxEvadc_disableQueueSlotExternalTrigger(Ifx_EVADC_G *evadcG, IfxEvadc_RequestSource sourceType)
{
    ...
    {
    case IfxEvadc_RequestSource_queue0:
    	/* disable external trigger for queue 0 */
        evadcG->Q[0].QMR.B.ENTR = 0;
        break;
    case IfxEvadc_RequestSource_queue1:
    	/* disable external trigger for queue 1 */
        evadcG->Q[1].QMR.B.ENTR = 0;
        break;
    case IfxEvadc_RequestSource_queue2:
    	/* disable external trigger for queue 2 */
        evadcG->Q[2].QMR.B.ENTR = 0;
        break;
    }
}
...
void IfxEvadc_enableQueueSlotExternalTrigger(Ifx_EVADC_G *evadcG, IfxEvadc_RequestSource sourceType)
{
    ...
    case IfxEvadc_RequestSource_queue0:
        ...
        evadcG->Q[0].QMR.B.ENTR = 1;
        break;
    case IfxEvadc_RequestSource_queue1:
    	/* enable external trigger */
        evadcG->Q[1].QMR.B.ENTR = 1;
        break;
    case IfxEvadc_RequestSource_queue2:
    	/* enable external trigger */
        evadcG->Q[2].QMR.B.ENTR = 1;
...
IfxEvadc_TriggerSource IfxEvadc_getQueueSlotTriggerInput(Ifx_EVADC_G *evadcG, IfxEvadc_RequestSource sourceType)
{
    ...
    {
    case IfxEvadc_RequestSource_queue0:
    	/* Retrieve trigger input source for queue 0 */
        triggerSource = evadcG->Q[0].QCTRL.B.XTSEL;
        break;
    case IfxEvadc_RequestSource_queue1:
    	/* Retrieve trigger input source for queue 1 */
        triggerSource = evadcG->Q[1].QCTRL.B.XTSEL;
        break;
    case IfxEvadc_RequestSource_queue2:
    	/* Retrieve trigger input source for queue 2 */
...
IfxEvadc_TriggerMode IfxEvadc_getQueueSlotTriggerMode(Ifx_EVADC_G *evadcG, IfxEvadc_RequestSource sourceType)
{
    ...
    {
    case IfxEvadc_RequestSource_queue0:
    	/* Retrieve trigger mode for queue 0 */
        triggerMode = evadcG->Q[0].QCTRL.B.XTMODE;
        break;
    case IfxEvadc_RequestSource_queue1:
    	/* Retrieve trigger mode for queue 1 */
        triggerMode = evadcG->Q[1].QCTRL.B.XTMODE;
        break;
    case IfxEvadc_RequestSource_queue2:
    	/* Retrieve trigger mode for queue 2 */
...
uint32 IfxEvadc_initializeFAdcI(Ifx_EVADC *evadc, uint32 fAdcI, Ifx_EVADC_G *group)
{
    ...
    /* Select frequency bounds based on supply voltage (GLOBCFG.SUPLEV) */
    if (evadc->GLOBCFG.B.SUPLEV == (uint32)IfxEvadc_SupplyVoltageLevelControl_lowerVoltage)
    {
        /* 3.3 V: 16 MHz .. 26.67 MHz */
        freqMin = IFXEVADC_ANALOG_FREQUENCY_MIN_3V3; 
        freqMax = IFXEVADC_ANALOG_FREQUENCY_MAX_3V3;
    }
    else
    {
         /* 5 V (automatic or upper): 16 MHz .. 53.33 MHz */
        freqMin = IFXEVADC_ANALOG_FREQUENCY_MIN_5V;    
        freqMax = IFXEVADC_ANALOG_FREQUENCY_MAX_5V;
    }

    /* Return 0 immediately if requested frequency is outside the valid range */
    if ((fAdcI < freqMin) || (fAdcI > freqMax))
    {
        return 0u;
    }
    
    divA   = (fadc << 2) / fAdcI;                          /* Scale by 4 to preserve sub-integer precision */
    divA   = (divA + 2) >> 2;                              /* Round to nearest integer: CP = round(fADC / fADCI) */
    divA   = (divA != 0u) ? __minu(divA - 1u, 0x1Fu) : 0u; /* DIVA = CP - 1; guard prevents uint32 underflow when CP=0 */
    result = fadc / __maxu(divA + 1u, 2u);                 /* fADCI = fADC / CP; CP = max(DIVA+1, 2) per hardware spec */

    if (!((result >= freqMin) && (result <= freqMax)))
    {
...
boolean IfxEvadc_isRequestQueueSlotEnabled(Ifx_EVADC_G *evadcG, IfxEvadc_RequestSource sourceType)
{
    ...
    {
    case IfxEvadc_RequestSource_queue0:
    	/* Check if queue 0 is enabled */
        enabled = evadcG->ARBPR.B.ASEN0;
        break;
    case IfxEvadc_RequestSource_queue1:
    	/* Check if queue 1 is enabled */
        enabled = evadcG->ARBPR.B.ASEN1;
        break;
    case IfxEvadc_RequestSource_queue2:
    	/* Check if queue 2 is enabled */
        enabled = evadcG->ARBPR.B.ASEN2;
        break;
    }

    /* Return the status of the specified queue slot */
    return enabled;
}
...
void IfxEvadc_setArbiterPriority(Ifx_EVADC_G *evadcG, boolean slotEnable, IfxEvadc_RequestSlotPriority prio, IfxEvadc_RequestSlotStartMode mode, IfxEvadc_RequestSource slot)
{
    if (slotEnable != FALSE)
    {
        ...
        {
        	/* Set cancel inject mode */
            evadcG->ARBPR.U |= 0x1u << (IFX_EVADC_G_ARBPR_CSM0_OFF + (slot * 4u));
        }
        else
        {
        	/* Set Wait for Start mode */
            evadcG->ARBPR.U &= ~(0x1u << (IFX_EVADC_G_ARBPR_CSM0_OFF + (slot * 4u)));
        }
    }
    else
    {
    	/* disable Slot */
        evadcG->ARBPR.U &= ~(IFX_EVADC_G_ARBPR_ASEN0_MSK << (IFX_EVADC_G_ARBPR_ASEN0_OFF + slot));
    }
}
...
boolean IfxEvadc_syncConvWorkAround(Ifx_EVADC *evadc)
{
    ...
    /* 0 = Operation failed.Used for detecting OCDS Debug Active State Activation failure & Trigger Line TL1 Activation failure. */
    boolean         status  = 1;
    ...
    {
    	/* trigger line TL1 forced to active */
        MODULE_CBS.TLC.B.TLSP1 = 0x3;
    }
    ...
    /* trigger line TL1 was not active */
    if (MODULE_CBS.TLS.B.TL1 == 0)
    {
    	/* trigger line TL1 forced inactive*/
        MODULE_CBS.TLC.B.TLSP1 = 0x2;
        /* trigger line TL1 unchanged (release forced inactive)*/
        MODULE_CBS.TLC.B.TLSP1 = 0x0;
    }

    /* OCDS is enabled*/
    if (MODULE_CBS.OSTATE.B.OEN == 1)
    {           /*disable OCDS*/
