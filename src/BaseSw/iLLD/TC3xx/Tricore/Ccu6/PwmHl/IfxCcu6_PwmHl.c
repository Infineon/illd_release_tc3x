 ...
 *       This function will set the ON time to ZERO, overriding previous call to IfxCcu6_PwmHl_setOnTime().
 *       This function will disable PWM in all channels, overriding previous call to IfxCcu6_PwmHl_setupChannels().
 *
 ...
 *
 * \param[in]    module Pointer to the base of CCU6 registers.
...
{
    return (driver->base.deadtime / driver->timer->base.clockFreq);
}


float32 IfxCcu6_PwmHl_getMinPulse(IfxCcu6_PwmHl *driver)
{
    return ( (driver->base.minPulse - driver->base.deadtime) / driver->timer->base.clockFreq );
}
...
boolean IfxCcu6_PwmHl_init(IfxCcu6_PwmHl *driver, const IfxCcu6_PwmHl_Config *config)
{
    ...
    /* only this mode is supported */
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, config->timer->base.countDir == IfxCcu6_Timer_CountDir_upAndDown);
    ...
    module->MCMCTR.B.SWSEL = IfxCcu6_MultiChannelSwitchingSelect_t12PeriodMatch;

    module->MCMCTR.B.SWSYN = IfxCcu6_MultiChannelSwitchingSync_t12ZeroMatch;
    ...
    module->T12DTC.U |= ( 1 << (IFX_CCU6_T12DTC_DTE0_OFF + IfxCcu6_T12Channel_0) );
    module->T12DTC.U |= ( 1 << (IFX_CCU6_T12DTC_DTE0_OFF + IfxCcu6_T12Channel_1) );
    module->T12DTC.U |= ( 1 << (IFX_CCU6_T12DTC_DTE0_OFF + IfxCcu6_T12Channel_2) );
    ...
    /* tOn do not need defined values */
...
{
	/* Default configuration for PWM HL */
    config->base.deadtime         = 0;
    config->base.minPulse         = 0;
    config->base.channelCount     = 0;
    config->base.emergencyEnabled = FALSE;
    config->base.outputMode       = IfxPort_OutputMode_pushPull;
    config->base.outputDriver     = IfxPort_PadDriver_cmosAutomotiveSpeed1;
    config->base.ccxActiveState   = Ifx_ActiveState_high;
    config->base.coutxActiveState = Ifx_ActiveState_high;
...
{
    Ifx_TimerValue value = deadtime * driver->timer->base.clockFreq;

    driver->base.deadtime = __minu(__minu(value, driver->timer->base.period / 2), 255);

    driver->timer->ccu6->T12DTC.B.DTM = (uint8)driver->base.deadtime;
    return TRUE;
}
...
{
    Ifx_TimerValue value = minPulse * driver->timer->base.clockFreq;
...
boolean IfxCcu6_PwmHl_setMode(IfxCcu6_PwmHl *driver, Ifx_Pwm_Mode mode)
{
	...
	/* only this mode is supported */
        ...
        {
	    /* If the mode is off, disable the PwmHl */
            IfxCcu6_PwmHl_disable(driver);
        }
        else
        {
	    /* If the mode is not off, enable the PwmHl */
...
void IfxCcu6_PwmHl_setOnTime(IfxCcu6_PwmHl *driver, Ifx_TimerValue *tOn)
{
    ...
    for (i = 0; i < channels; i++)
    {
        ...
        module->CC6SR[(IfxCcu6_T12Channel)i].B.CCS = (uint16)cmp;
    }
}
...
void IfxCcu6_PwmHl_setupChannels(IfxCcu6_PwmHl *driver, boolean *activeCh, boolean *stuckSt)
{
    ...
    Ifx_CCU6                 *module = timer->ccu6;
    {
        ...
        module->MCMOUTS.B.MCMPS = pattern;
    }
    {
        Ifx_CCU6_PSLR pslr;
