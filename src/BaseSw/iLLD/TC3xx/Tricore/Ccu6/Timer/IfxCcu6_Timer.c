...
void IfxCcu6_Timer_initModule(IfxCcu6_Timer *timer, const IfxCcu6_Timer_Config *config)
{
    ...
    {
        IfxCcu6_Timer_initTimer13(config);
    }

    /* Timer 12 initialisation */
    if ((config->timer == IfxCcu6_TimerId_t12) || (config->trigger.t13InSyncWithT12) || (config->synchronousOperation))
    {
        IfxCcu6_Timer_initTimer12(config);
    }

    /* -- Pin mapping -- */
    if (config->pins != NULL_PTR)
    {
        IfxCcu6_Timer_initPins(config->pins);
    }

    /* -- interrupt initialisation -- */
    IfxCcu6_Timer_initInterrupt(config->ccu6, config->interrupt1);
    IfxCcu6_Timer_initInterrupt(config->ccu6, config->interrupt2);
    IfxCcu6_Timer_initInterrupt(config->ccu6, config->interrupt3);
    IfxCcu6_Timer_initInterrupt(config->ccu6, config->interrupt4);
...
void IfxCcu6_Timer_initTimer13(const IfxCcu6_Timer_Config *config)
{
    Ifx_CCU6 *ccu6SFR = config->ccu6;
    ...
    if (config->clock.t13ExtClockEnabled)   /* if external source is selected as reference */
    {
        ...
        /* period setting */
        ccu6SFR->T13PR.B.T13PV = (uint16)config->base.t13Period;
    }
...
void IfxCcu6_Timer_initTimer12(const IfxCcu6_Timer_Config *config)
{
    Ifx_CCU6 *ccu6SFR = config->ccu6;
...
void IfxCcu6_Timer_initPins(const IfxCcu6_Timer_Pins *pins)
{
    /* -- Pin mapping -- */
...
void IfxCcu6_Timer_initInterrupt(Ifx_CCU6 *ccu6, IfxCcu6_Timer_InterruptConfig interrupt)
{
    /* -- interrupt initialisation -- */
...
void IfxCcu6_Timer_startSingleShotMode(IfxCcu6_Timer *timer)
{
    ...
    if (timer->timer == IfxCcu6_TimerId_t13)
    {
        ...
        ccu6SFR->TCTR2.U = ccu6SFR->TCTR2.U | (1U << IfxCcu6_TimerId_t13);
    ...
    if ((timer->timer == IfxCcu6_TimerId_t12) || (timer->trigger.t13InSyncWithT12))
    {
        ...
        /* enable single shot mode */
        ccu6SFR->TCTR2.U = ccu6SFR->TCTR2.U | (1U << IfxCcu6_TimerId_t12);
...
void IfxCcu6_Timer_stop(IfxCcu6_Timer *timer)
{
    ...
    if ((timer->timer == IfxCcu6_TimerId_t13) || (timer->trigger.t13InSyncWithT12))
    {
        ...
        if (timer->trigger.t13InSyncWithT12)
        {
            timer->ccu6->TCTR2.B.T13TEC = IfxCcu6_T13TriggerEvent_noAction;
            timer->ccu6->TCTR2.B.T13TED = IfxCcu6_T13TriggerDirection_noAction;
        }
