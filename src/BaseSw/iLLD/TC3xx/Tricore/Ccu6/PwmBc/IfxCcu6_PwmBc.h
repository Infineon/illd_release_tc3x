 ...
 *
 * \param[inout] pwmBc  Pointer to the IfxCcu6_PwmBc module handle.
...
/**
 * \brief Initializes the timer13 configuration and returns the frequency.
 *
 * \param[inout] config  Pointer to the configuration structure to be initialized.
 *
 * \retval float32 The frequency of the timer after initialization.
 */
IFX_EXTERN float32 IfxCcu6_PwmBc_initTimer13(const IfxCcu6_PwmBc_Config *config);

/**
 * \brief Initializes the Timer12 configuration for the PWM module and returns the operating frequency.
 *
 * \param[inout] config Pointer to the configuration structure to be initialized.
 *
 * \retval float32 The frequency at which Timer12 is operating after initialization.
 */
IFX_EXTERN float32 IfxCcu6_PwmBc_initTimer12(const IfxCcu6_PwmBc_Config *config);

/**
 * \brief Initializes the channel configuration for the PWM BC module.
 *
 * \param[inout] config Pointer to the configuration structure to be initialized.
 *
 * \retval None
 */
IFX_EXTERN void IfxCcu6_PwmBc_initChannelConfig(const IfxCcu6_PwmBc_Config *config);

/**
 * \brief Initializes the output path configuration for the CCU6 PWM module.
 *
 * \param[inout] ccu6SFR  Pointer to the base of CCU6 registers.
 * \param[in]    pins     Pointer to the configuration structure of type IfxCcu6_PwmBc_Pins, which contains the pin settings.
 *
 * \retval None
 */
IFX_EXTERN void IfxCcu6_PwmBc_initOutputPaths(Ifx_CCU6 *ccu6SFR, IfxCcu6_PwmBc_Pins *pins);

/**
 * \brief Initializes the CCU6 pins according to the provided configuration.
 *
 * \param[in] pins  Pointer to the configuration structure of type IfxCcu6_PwmBc_Pins, which contains the pin settings.
 *
 * \retval None
 */
IFX_EXTERN void IfxCcu6_PwmBc_initPins(const IfxCcu6_PwmBc_Pins *pins);

/**
 * \brief Initializes the CCU6 channel interrupts with the specified configuration.
 *
 * \param[inout] ccu6       Pointer to the base of CCU6 registers.
 * \param[in]    interrupt  Configuration structure containing interrupt settings such as enable flags, priority configuration, and service request node selection.
 *
 * \retval None
 */
IFX_EXTERN void IfxCcu6_PwmBc_initInterrupt(Ifx_CCU6 *ccu6, IfxCcu6_PwmBc_InterruptConfig interrupt);

/**
 * \brief Initializes the PWMBC module configuration structure with default values.
 *
 * \param[inout] config   Pointer to the configuration structure to be initialized.
...
/**
 * \brief Starts the Hall sensor-based PWM functionality.
 *
 * \param[inout] pwmBc Pointer to the IfxCcu6_PwmBc module handle.
 *
 * \retval None
...
/**
 * \brief Stops the Hall sensor-based PWM operation.
 *
 * \param[inout] pwmBc Pointer to the IfxCcu6_PwmBc module handle.
 *
 * \retval None
 ...
 *
 * \param[in] pwmBc Pointer to the IfxCcu6_PwmBc module handle.
 *
 * \retval uint32 The current motor speed
 ...
 *
 * \param[inout] pwmBc        Pointer to the IfxCcu6_PwmBc module handle.
 * \param[in]    controlTable 2D array of uint8 values, representing the Motor Control control table for the Hall pattern.
 *
 * \retval None
 */
