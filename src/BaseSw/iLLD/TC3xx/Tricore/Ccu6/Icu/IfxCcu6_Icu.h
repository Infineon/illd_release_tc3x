...
/**
 * \brief Initializes the ICU module with the default configuration.
 *
 * \param[inout]  icu     Pointer to the IfxCcu6_Icu module handle.
...
/**
 * \brief Initializes the ICU module configuration structure with default values.
 *
 * \param[inout]  config  Pointer to the configuration structure to be initialized.
...
/**
 * \brief Returns the current timestamp value stored in the Compare Shadow register.
 *
 * \param[in] channel Pointer to the channel handle structure that will be initialized.
 *
 * \retval uint32 The current timestamp value (CC6xSR register value) as a 32-bit unsigned integer.
 *                The value represents the current time stamp captured by the ICU channel.
 *                Range: 0x0001 to 0x7FFF.
 */
IFX_EXTERN uint32 IfxCcu6_Icu_getTimeStamp(IfxCcu6_Icu_Channel *channel);

/**
 * \brief Starts the input capture operation on the specified channel.
 *
 * \param[inout] channel  Pointer to the channel handle structure that will be initialized.
 *
 * \retval None
 */
...
/**
 * \brief Stops the input capture for the specified ICU channel.
 *
 * \param[inout] channel Pointer to the channel handle structure that will be initialized.
 *
 * \retval None
...
/**
 * \brief Initializes a CCU6 ICU channel with the specified configuration.
 *
 * \param[inout] channel       Pointer to the channel handle structure that will be initialized.
 * \param[inout] channelConfig Pointer to the configuration structure containing the parameters for the channel initialization.
 *
 * \retval None
 */
...
/**
 * \brief Initializes the channel configuration structure with default values for a specific channel.
 *
 * \param[inout] channelConfig  Pointer to the configuration structure containing the parameters for the channel initialization.
 * \param[in]    ccu6           Pointer to the base of CCU6 registers.
 *
...
/**
 * \brief Initializes the channel pins for the CCU6 ICU module.
 *
 * \param[in] pins                     Pointer to the configuration structure of type IfxCcu6_Icu_Pins, which contains the pin settings.
 * \param[in] multiInputCaptureEnabled Specifies whether multi-input capture mode is enabled.
 *                                     Range:  TRUE  : Multi-input capture mode is enabled.
 *        									   FALSE : Multi-input capture mode is disabled.
 *
 * \retval None
 */
...
/**
 * \brief Initializes the channel interrupts for the CCU6 ICU module.
 *
 * \param[inout] ccu6      Pointer to the base of CCU6 registers.
 * \param[in]    interrupt Pointer to the structure containing the interrupt configuration.
 *
 * \retval None
 */
