...
#ifdef DEVICE_TC35X
    case IfxMtu_MbistSel_ememUpper0:
        systemAddress = 0x99000000 | (sramAddress << 5);
        break;
    case IfxMtu_MbistSel_ememUpper1:
        systemAddress = 0x99100000 | (sramAddress << 5);
        break;
#endif
...
uint8 IfxMtu_runNonDestructiveInversionTest(IfxMtu_MbistSel mbistSel, uint8 rangeSel, uint8 rangeAddrUp, uint8 rangeAddrLow, uint16 *errorAddr)
{
    ...
    uint32 mbistSelPspr[IFXCPU_NUM_MODULES * 2] = {IfxMtu_MbistSel_cpu0Pspr, IfxMtu_MbistSel_cpu0Ptag,
                                                   ...
                                                   IfxMtu_MbistSel_cpu4Pspr, IfxMtu_MbistSel_cpu4Ptag,
                                                   IfxMtu_MbistSel_cpu5Pspr, IfxMtu_MbistSel_cpu5Ptag
											       };
...
#endif
    for (i = 0; i < (IFXCPU_NUM_MODULES * 2); i++)
    {
        if (mbistSel == mbistSelPspr[i])
        {
            /* Check if the Endinit is cleared by application. If not, then handle it internally inside teh function.*/
            if (IfxScuWdt_getSafetyWatchdogEndInit() == 1U)
            {
