 ...
 *
 * \param[inout] driver Driver handle.
 *
 * \retval None
 */
...
/** \brief Update raw position based on current timer value and offset.
 *
 * \param[inout] driver Driver handle.
 *
 * \retval None
 */
IFX_STATIC void IfxGpt12_IncrEnc_updateRawPosition(IfxGpt12_IncrEnc *driver);
...
{
    sint32 wrappedOffset;

    /* Normalize offset: wrap negative and oversized offsets into [0, resolution-1] */
	wrappedOffset = offset % driver->resolution;
	if (wrappedOffset < 0)
	{
		wrappedOffset += driver->resolution;
	}

    driver->offset          = wrappedOffset;
    driver->notSynchronised = 0;

    /* Refresh raw position */
    IfxGpt12_IncrEnc_updateRawPosition(driver);
}
...
IFX_STATIC void IfxGpt12_IncrEnc_updateRawPosition(IfxGpt12_IncrEnc *driver)
{
    ...
    sint32 	   newPosition = 0;

    /* Determine which timer is configured in incremental interface mode */
    boolean t2ModeIncremental = (gpt12->T2CON.B.T2M == IfxGpt12_Mode_incrementalInterfaceEdgeDetection);
    boolean t3ModeIncremental = (gpt12->T3CON.B.T3M == IfxGpt12_Mode_incrementalInterfaceRotationDetection);
    boolean t4ModeIncremental = (gpt12->T4CON.B.T4M == IfxGpt12_Mode_incrementalInterfaceEdgeDetection);

    if (t2ModeIncremental)
    {
    	newPosition = gpt12->T2.U;
    }
    else if (t3ModeIncremental)
    {
    	newPosition = gpt12->T3.U;
    }
    else if (t4ModeIncremental)
    {
    ...
    /* Wrap position into valid range [0, resolution-1]
     * Note: newPosition is always >= 0 since both base and offset are non-negative */
