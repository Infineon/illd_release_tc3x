 ...
 */
IFX_STATIC uint32 IfxSdmmc_Sd_FuncBlockSize[IFXSDMMC_BLOCKSIZE_8] = {0};
...
IfxSdmmc_Status IfxSdmmc_Sd_initCard(IfxSdmmc_Sd *sd, IfxSdmmc_Sd_CardConfig *cardConfig)
{
    ...
    status = IfxSdmmc_Sd_ioInit(sd, cardConfig);

    if(status == IfxSdmmc_Status_success)
    {
        IfxSdmmc_Sd_configureSpeedAndBusWidth(sd, cardConfig);
    }
    ...
    if (status == IfxSdmmc_Status_success)
    {
        if (sd->cardType != IfxSdmmc_SdCardType_io)
        {
            ...
            status = IfxSdmmc_sendCommand(sdPtr, IfxSdmmc_Command_setBlockLength, IFXSDMMC_BLOCKSIZE_512, IfxSdmmc_ResponseType_r1, &response);
        }
...
IfxSdmmc_Status IfxSdmmc_Sd_ioInit(IfxSdmmc_Sd *sd, IfxSdmmc_Sd_CardConfig *cardConfig)
{
    IfxSdmmc_Status status = IfxSdmmc_Status_success;
    ...
    /* Reset the card (CMD0) */
    status = IfxSdmmc_sendCommand(sdPtr, IfxSdmmc_Command_goIdleState, IFXSDMMC_ARG_NONE, IfxSdmmc_ResponseType_none, NULL_PTR);
...
IfxSdmmc_Status IfxSdmmc_Sd_configureSpeedAndBusWidth(IfxSdmmc_Sd *sd, IfxSdmmc_Sd_CardConfig *cardConfig)
{
    IfxSdmmc_Status status = IfxSdmmc_Status_success;
    ...
    if (status == IfxSdmmc_Status_success)
    {
        ...
        if (sd->presetMode == TRUE)
        {
            ...
            sdPtr->HOST_CTRL2.B.PRESET_VAL_ENABLE = 1; /* Enable preset values */
            sdPtr->CLK_CTRL.B.SD_CLK_EN = 1; /* Enable card clock */
        }
        else
        {
            IfxSdmmc_switchClockFrequency(sdPtr, sd->userFrequency);
        }
    }
...
void IfxSdmmc_Sd_configureInterrupt(IfxSdmmc_Sd *sd, IfxSdmmc_InterruptConfig *interruptConfig)
{
...
void IfxSdmmc_Sd_initModuleConfig(IfxSdmmc_Sd_Config *config, Ifx_SDMMC *sdmmcSFR)
{
    const IfxSdmmc_Sd_Config defaultConfig = {
        ...
        .hostConfig = {
            ...
            .frequency       = IFXSDMMC_FREQUENCY_10MHZ,
...
IfxSdmmc_Status IfxSdmmc_Sd_readCid(IfxSdmmc_Sd *sd)
{
    ...
    status              = IfxSdmmc_sendCommand(sdPtr, IfxSdmmc_Command_allSendCID, IFXSDMMC_ARG_NONE, IfxSdmmc_ResponseType_r2, &response);
...
IfxSdmmc_Status IfxSdmmc_Sd_readRca(IfxSdmmc_Sd *sd)
{
    ...
    if (status == IfxSdmmc_Status_success)
    {
        sd->cardInfo.rca = (uint16)((response.resp01 & 0xFFFF0000) >> IFXSDMMC_BITSHIFT_POS16);
    }
    else
    {
...
IfxSdmmc_Status IfxSdmmc_Sd_readScr(IfxSdmmc_Sd *sd)
{
    ...
    if (status == IfxSdmmc_Status_success)
    {
        sdPtr->XFER_MODE.B.DATA_XFER_DIR = IfxSdmmc_TransferDirection_read; /* Set transfer direction */
        status           = IfxSdmmc_Sd_singleBlockTransfer(sd, IfxSdmmc_Command_sendSCR, argument, IFXSDMMC_BLOCKSIZE_8, (uint32 *)&scr, IfxSdmmc_TransferDirection_read);
        sd->cardInfo.scr = scr;
    }
...
IFX_STATIC IfxSdmmc_Status IfxSdmmc_Sd_setVoltageWindow(IfxSdmmc_Sd *sd)
{
    ...
    do
    {
        ...
        if (status == IfxSdmmc_Status_success)
        {
            ...
            status           = IfxSdmmc_sendCommand(sdPtr, IfxSdmmc_Command_sdSendOpCond, argument, IfxSdmmc_ResponseType_r3, &response);
            ...
            {
                retryLoopCtr = (IFXSDMMC_RETRYDELAY_ACMD41_US * IFXSDMMC_1KMULTIPLIER) / IFXSDMMC_1KLOOPDELAY_US;
                uint32 index;
...
IfxSdmmc_Status IfxSdmmc_Sd_singleBlockAdma2Transfer(IfxSdmmc_Sd *sd, IfxSdmmc_Command command, uint32 address, uint16 blockSize, uint32 *descrAddress, IfxSdmmc_TransferDirection direction)
{
    ...
    sdPtr->XFER_MODE.B.MULTI_BLK_SEL = FALSE; /* Reset multi-block select */
    ...
    {
        sdPtr->XFER_MODE.B.DATA_XFER_DIR = IfxSdmmc_TransferDirection_write;
    }

    status = IfxSdmmc_sendCommand(sdPtr, command, address, IfxSdmmc_ResponseType_r1, &response);
    ...
    if (status == IfxSdmmc_Status_success)
    {
        ...
        if ((timeout == 0) || (IfxSdmmc_isErrorInterruptOccured(sdPtr, IfxSdmmc_ErrorInterrupt_adma) != 0))
        {
            status = IfxSdmmc_Status_dataError;

            if (IfxSdmmc_isErrorInterruptOccured(sdPtr, IfxSdmmc_ErrorInterrupt_adma) != 0)
            {
                IfxSdmmc_clearErrorInterrupt(sdPtr, IfxSdmmc_ErrorInterrupt_adma);
            }

        }
        else
        {
            IfxSdmmc_clearNormalInterrupt(sdPtr, IfxSdmmc_NormalInterrupt_transferComplete);
        }
    }
...
IfxSdmmc_Status IfxSdmmc_Sd_singleBlockTransfer(IfxSdmmc_Sd *sd, IfxSdmmc_Command command, uint32 address, uint16 blockSize, uint32 *data, IfxSdmmc_TransferDirection direction)
{
    ...
    if (status == IfxSdmmc_Status_success)
    {
        if (direction == IfxSdmmc_TransferDirection_read)
        {
            ...
            {
                IfxSdmmc_clearNormalInterrupt(sdPtr, IfxSdmmc_NormalInterrupt_bufferReadReady);
            }
        }
    }
    ...
    if (status == IfxSdmmc_Status_success)
    {
        ...
        for (loopIndex = 0U; loopIndex < numWords; loopIndex++)
        {
            if (direction == IfxSdmmc_TransferDirection_read)
            {
                *data = (uint32)sdPtr->BUF_DATA.U;
            }

            if (direction == IfxSdmmc_TransferDirection_write)
            {
                sdPtr->BUF_DATA.U = *data;
            }
...
IfxSdmmc_Status IfxSdmmc_Sd_switchToBusWidth4(IfxSdmmc_Sd *sd)
{
    ...
    if (sd->flags.memInit)  /* for Mem cards */
    {
        ...
        {
            argument |= sd->cardInfo.rca << IFXSDMMC_BITSHIFT_POS16;
            status    = IfxSdmmc_sendCommand(sdPtr, IfxSdmmc_Command_appCommand, argument, IfxSdmmc_ResponseType_r1, &response);

            if (status == IfxSdmmc_Status_success)
            {
                status = IfxSdmmc_sendCommand(sdPtr, IfxSdmmc_Command_setBusWidth, IFXSDMMC_ARG_SD_BUSWIDTH_4, IfxSdmmc_ResponseType_r1, &response);
            }
        }
    }
    ...
    if (status == IfxSdmmc_Status_success)
    {
        sdPtr->HOST_CTRL1.B.DAT_XFER_WIDTH = IfxSdmmc_SdDataTransferWidth_4Bit;
    }
    else
    {
        sdPtr->HOST_CTRL1.B.DAT_XFER_WIDTH = IfxSdmmc_SdDataTransferWidth_1Bit;
    }
...
IfxSdmmc_Status IfxSdmmc_Sd_switchToHighSpeed(IfxSdmmc_Sd *sd)
{
    ...
    if (sd->flags.memInit) /* for memory cards */
    {
        ...
        if (sd->cardInfo.scr.B.sdSpec != 0)
        {
            ...
            if (status == IfxSdmmc_Status_success)
            {
                status = IfxSdmmc_Sd_singleBlockTransfer(sd, IfxSdmmc_Command_switchFunction, IFXSDMMC_ARG_SD_HIGHSPEED, IFXSDMMC_BLOCKSIZE_64, (uint32 *)switchStatus, IfxSdmmc_TransferDirection_read);
    ...
    if (status == IfxSdmmc_Status_success)
    {
        sdPtr->HOST_CTRL1.B.HIGH_SPEED_EN = IfxSdmmc_SdSpeedMode_high;
    }
    else
    {
        sdPtr->HOST_CTRL1.B.HIGH_SPEED_EN = IfxSdmmc_SdSpeedMode_normal;
    }
...
IFX_STATIC IfxSdmmc_Status IfxSdmmc_Sd_validateInterfaceCondition(IfxSdmmc_Sd *sd)
{
    ...
    argument |= (IFXSDMMC_HOST_SUPPORTED_VOLTAGE << IFXSDMMC_BITSHIFT_POS8) | (IFXSDMMC_IF_COND_CHECK_PATTERN);
    status    = IfxSdmmc_sendCommand(sdPtr, IfxSdmmc_Command_sendIfCond, argument, IfxSdmmc_ResponseType_r7, &response);
...
IFX_STATIC IfxSdmmc_Status IfxSdmmc_Sd_validateOperatingCondition(IfxSdmmc_Sd *sd)
{
    ...
    if ((status == IfxSdmmc_Status_success) || ((sd->flags.f8 == 0U) && (status == IfxSdmmc_Status_commandError)))
    {
        ...
        status           = IfxSdmmc_sendCommand(sdPtr, IfxSdmmc_Command_sdSendOpCond, IFXSDMMC_RCA_NONE, IfxSdmmc_ResponseType_r3, &response);
        sd->cardInfo.ocr = response.resp01;
        ...
        if ((status == IfxSdmmc_Status_commandError) || (status == IfxSdmmc_Status_success))
        {
            ...
            if (status == IfxSdmmc_Status_success)
            {
                ...
                else
                {
                    ...
                    if ((sd->cardInfo.ocr & IFXSDMMC_STATUS_HIGHCAPACITYCARD) != 0U)
                    {
...
IfxSdmmc_Status IfxSdmmc_Sd_ioSetVoltageWindow(IfxSdmmc_Sd *sd)
{
    ...
    while ((ioInfo.bits.c != (uint8)IfxSdmmc_SdIoInit_ready) && (loopCount > 0))
    {
        ...
        else
        {
            ...
            /* This command to be sent every 3ms for 1 second */
            {
                retryLoopCtr = (IFXSDMMC_SDIO_RETRYDELAY_CMD5_US * IFXSDMMC_1KMULTIPLIER) / IFXSDMMC_1KLOOPDELAY_US;
                uint32 index;
...
IfxSdmmc_Status IfxSdmmc_Sd_ioReadRegister(IfxSdmmc_Sd *sd, IfxSdmmc_SdIoFunction func, uint32 addr, uint8 *reg)
{
    ...
    {
        /* Send the command */
        status = IfxSdmmc_sendCommand(sdPtr, IfxSdmmc_Command_ioRwDirect, argument.arg, IfxSdmmc_ResponseType_r5, &response);
...
IfxSdmmc_Status IfxSdmmc_Sd_ioBlockTransfer(IfxSdmmc_Sd *sd, uint32 argument, uint16 blockSize, uint32 blockCount, uint32 *data, IfxSdmmc_TransferDirection direction)
{
    ...
    sdPtr->BLOCKSIZE.B.XFER_BLOCK_SIZE = blockSize; /* Set block size */

    if (blockCount > 1)
    {
        sdPtr->XFER_MODE.B.MULTI_BLK_SEL = TRUE; /* Set multi-block select */
        IfxSdmmc_setBlockCount(sdPtr, blockCount);
        sdPtr->XFER_MODE.B.BLOCK_COUNT_ENABLE = 1; /* Enable block count */
    }
    else
    {
        sdPtr->XFER_MODE.B.MULTI_BLK_SEL = FALSE; /* Reset multi-block select */
    ...
    while (numBlocks > 0)   /* Iterate over all the blocks */
    {
        if (status == IfxSdmmc_Status_success)
        {
            ...
            else if ((direction == IfxSdmmc_TransferDirection_write) && (numBlocks < blockCount))
            {
                ...
                {
                    IfxSdmmc_clearNormalInterrupt(sdPtr, IfxSdmmc_NormalInterrupt_bufferWriteReady);
                }
            }
        }
...
IfxSdmmc_Status IfxSdmmc_Sd_readMultiBlock(IfxSdmmc_Sd *sd, uint32 address, uint32 *data, uint32 blockCount, IfxSdmmc_BlockBoundarySize blockBoundarySize)
{
    IfxSdmmc_Status status  = IfxSdmmc_Status_success;
    uint32          timeout = 0;
    /* Local pointer for sd->sdmmcSFR */
    ...
    else
    {
        ...
        if (sd->dmaUsed == TRUE)
        {
            status = IfxSdmmc_Sd_multiBlockDmaTransfer(sd, IfxSdmmc_Command_readMultipleBLock, address, IFXSDMMC_BLOCK_SIZE_DEFAULT, data, IfxSdmmc_TransferDirection_read, blockCount, blockBoundarySize);
        }
        else
        {
...
IfxSdmmc_Status IfxSdmmc_Sd_multiBlockDmaTransfer(IfxSdmmc_Sd *sd, IfxSdmmc_Command command, uint32 address, uint16 blockSize, uint32 *data, IfxSdmmc_TransferDirection direction, uint32 blockCount, IfxSdmmc_BlockBoundarySize blockBoundarySize)
{
    IfxSdmmc_Status   status  = IfxSdmmc_Status_success;
    IfxSdmmc_Response response;
    uint32            timeout = 0;
    ...
    sdPtr->BLOCKSIZE.B.XFER_BLOCK_SIZE = blockSize;
    sdPtr->BLOCKSIZE.B.SDMA_BUF_BDARY = blockBoundarySize; /* Set boundary size */
    sdPtr->XFER_MODE.B.MULTI_BLK_SEL = TRUE; /* Set multi-block select */
...
IfxSdmmc_Status IfxSdmmc_Sd_writeMultiBlock(IfxSdmmc_Sd *sd, uint32 address, uint32 *data, uint32 blockCount, IfxSdmmc_BlockBoundarySize blockBoundarySize)
{
    IfxSdmmc_Status status  = IfxSdmmc_Status_success;
    uint32          timeout = 0;
    /* Local pointer for sd->sdmmcSFR */
    ...
    else
    {
        ...
        if (sd->dmaUsed == TRUE)
        {
            ...
            {
                status = IfxSdmmc_Sd_multiBlockDmaTransfer(sd, IfxSdmmc_Command_writeMultipleBlock, address, IFXSDMMC_BLOCK_SIZE_DEFAULT, data, IfxSdmmc_TransferDirection_write, blockCount, blockBoundarySize);
            }
        }
        else
        {
