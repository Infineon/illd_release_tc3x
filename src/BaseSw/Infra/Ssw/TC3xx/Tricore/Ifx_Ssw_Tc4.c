...
void __Core4_start(void)
{
    ...
    unsigned short wdtPassword   = Ifx_Ssw_getCpuWatchdogPasswordInline(&MODULE_SCU.WDTCPU[4]);

    /* Set the PSW to its reset value in case of a warm start,clear PSW.IS */
    Ifx_Ssw_MTCR(CPU_PSW, IFX_CFG_SSW_PSW_DEFAULT);

    /* Clear the ENDINIT bit in the WDT_CON0 register, inline function */
    Ifx_Ssw_clearCpuEndinitInline(&MODULE_SCU.WDTCPU[4], wdtPassword);
    ...
    Ifx_Ssw_setCpuEndinitInline(&MODULE_SCU.WDTCPU[4], wdtPassword);
...
void _START4(void)
{
	Ifx_Ssw_Start(__USTACK(4), __Core4_start);
}
