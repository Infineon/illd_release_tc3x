...
{    
    ...
    edmem: ORIGIN=0x99000000, LENGTH=2M
    edmem_nc: ORIGIN=0xb9000000, LENGTH=2M
}
...
SECTIONS
{
    ...
    .traptab_tc1 (LCF_TRAPVEC1_START) : { PROVIDE(__TRAPTAB_CPU1 = .); *(.traptab_cpu1) . = align(32);} > pfls0
    
    /* Fixed memory Allocations for _START  for CPU1 to 2*/
    .start_tc1 (LCF_STARTPTR_NC_CPU1) : AT(LCF_STARTPTR_CPU1) { PROVIDE(_start_cpu1 = .); *(.start_cpu1) } > pfls0_nc
    .start_res1 (LCF_STARTPTR_CPU1 + SIZEOF(.start_tc1)) : {} > pfls0
    ...
    .CPU1_rodata          ALIGN(4) : { "Ifx_Ssw_Tc1.*(.rodata)" "Cpu1_Main.*(.rodata)" "*(.rodata_cpu1)" } > pfls0
    ...
    .CPU1_text            ALIGN(2) : { "Ifx_Ssw_Tc1.*(.text)" "Cpu1_Main.*(.text)" "*(.text_cpu1)" } > pfls0
    ...
    /* Interrupt vector table located at PFLS0 */
    .inttab_tc1_000 BIND (__INTTAB_CPU1 + 0x0000) ALIGN(8) : { *(.intvec_tc1_0  ) *(.intvec_tc1_0x0 ) *(.intvec_tc1_0x0 ) } > pfls0
