...
{
    /*For TC33xED there is no need to configure the EVR, The UM mentions to use 
...
void Ifx_Ssw_Lbist(void)
{
    ...
    if (!IfxScuLbist_evaluateResult(IfxScuLbist_defaultConfig.signature))
    {
         ...
         while(1);   /* infinite loop to ensure that error is notified as debugger is not connected at this point of time */
    }
